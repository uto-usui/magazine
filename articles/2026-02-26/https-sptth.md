---
title: "ローカル HTTPS 開発専用ツール SPTTH を公開した"
source: "https://blog.jxck.io/entries/2026-02-25/sptth.html"
...
[http://localhost:3000](http://localhost:3000/) での開発には限界がある。

しかし、本番と同じように [https://example.com](https://example.com/) でアクセスできる環境をローカルに作るには、ドメインの解決、証明書の発行、443 での起動など、少し手間がかかる。

そこで、必要な全てを 1 つのツールで行い、様々な開発環境を再現するためのツールを開発したので、紹介する。

![sptth logo](https://blog.jxck.io/entries/2026-02-25/logo.banner.svg?260226_015344)

-   Jxck/sptth: reverse https proxy (https - sptth) for local development
    -   [https://github.com/Jxck/sptth](https://github.com/Jxck/sptth)

## [localhost の罠](#localhost-の罠)
...
つまり、ある程度複雑なサービスを開発していれば、[http://localhost:3000](http://localhost:3000/) での開発など、早々に限界を迎えるのだ。

もし本番が [https://example.com](https://example.com/) ならば、ローカルでも同じく [https://example.com](https://example.com/) で開発したい。
...
手法自体は知られている。以下の 3 つを行えば、ローカルで起動したサーバに [https://example.com](https://example.com/) でアクセスできるようになる。
...
ブラウザから [https://example.com](https://example.com/) にアクセスすれば [http://localhost:3000](http://localhost:3000/) に、[https://example.net](https://example.net/) にアクセスすれば [http://localhost:4000](http://localhost:4000/) にアクセスできる。ローカル CA で署名した正しい証明書なので、URL バーも赤くならない。
...
[https://github.com/Jxck/sptth/releases](https://github.com/Jxck/sptth/releases)
