...
source: "https://kentcdodds.com/blog/implementing-hybrid-semantic-lexical-search"
publishedDate: "2026-03-16"
category: "frontend"
feedName: "Kent C. Dodds"
---

A few weeks ago I [added semantic search to kentcdodds.com](https://kentcdodds.com/blog/building-semantic-search-on-my-content). It lets you search with natural-language questions like ["How did Kent get his first job?"](https://kentcdodds.com/search?q=How+did+Kent+get+his+first+job?) or ["What's the best way to learn React?"](https://kentcdodds.com/search?q=What%27s+the+best+way+to+learn+React?) and get genuinely relevant results instead of a keyword-match list. I was pretty happy with it.

But it wasn't good enough.

The most glaring failure: searching for "React Testing Library" didn't even surface the original [introduction post](https://kentcdodds.com/blog/introducing-the-react-testing-library). That's the canonical piece on the topic. It should be the first result. Pure semantic search missed it entirely because the embedding model weighted the conceptual meaning of "React Testing Library" over the literal title match.

What followed was three rounds of design, build, ship, regret, and redesign. This post is about all of it.

## [The Problem With Pure Vector Search](#the-problem-with-pure-vector-search)
...
Natural language, intent, paraphrase

Exact identifiers, proper nouns

Lexical

Exact terms, titles, API names

Fuzzy/intent queries, synonyms
...
## [Round 1: Building Hybrid Search](#round-1-building-hybrid-search)
...
### [The workflow with Cursor and GPT-5.4](#the-workflow-with-cursor-and-gpt-54)
...
### [A bug Agent Review caught](#a-bug-agent-review-caught)
...
## [Round 2: The Production Failure](#round-2-the-production-failure)
...
### [Fixing it: a dedicated Cloudflare Worker](#fixing-it-a-dedicated-cloudflare-worker)
...
## [Why That Still Bothered Me](#why-that-still-bothered-me)
...
## [Round 3: One Search Worker](#round-3-one-search-worker)
...
D1 FTS5Search WorkerGitHub ActionsVectorizeR2 lexical artifactsIndexersContent sourcesD1 FTS5Search WorkerGitHub ActionsVectorizeR2 lexical artifactsIndexersContent sourcesLoad source content1Upsert vectors + metadata2Write lexical artifacts3POST /internal/sync4Read lexical artifacts5Replace docs, chunks, and FTS rows6Update lastSyncedAt7

And here's the runtime query flow when someone actually performs a search:

VectorizeWorkers AID1 FTS5Search WorkerSite route/searchKCDUserVectorizeWorkers AID1 FTS5Search WorkerSite route/searchKCDUserpar\[In parallel\]alt\[Cache miss\]Search request1GET /health2Read lastSyncedAt3syncedAt4Build cache key5POST /search6Run lexical query7Lexical matches8Generate embedding9Query vector10Run semantic query11Semantic matches12Fuse and rank13Search results14Add summary/image15Return results16

### [Lexical artifacts](#lexical-artifacts)
...
### [The search worker](#the-search-worker)
...
### [The site boundary](#the-site-boundary)
