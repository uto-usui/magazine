---
title: "HTTP 関連 RFC が大量に出た話と 3 行まとめ"
source: "https://blog.jxck.io/entries/2022-06-16/HTTP-RFCs.html"
...
### [HTTP 仕様のリファクタリング](#http-仕様のリファクタリング)
...
### [RFC 9112: HTTP/1.1](#rfc-9112-http1.1)
...
### [RFC 9163: Expect-CT Extension for HTTP](#rfc-9163-expect-ct-extension-for-http)
...
### [RFC 9204: QPACK: Field Compression for HTTP/3](#rfc-9204-qpack-field-compression-for-http3)

-   HTTP/3 の Field を圧縮するための仕様、HPACK の後継。
-   HPACK よりも事前定義 Field のリストが増えた。
-   そこから選ぶと 1byte で送れるので知っておくと良さそう。

### [RFC 9205: Building Protocols with HTTP](#rfc-9205-building-protocols-with-http)
...
### [RFC 9213: Targeted HTTP Cache Control](#rfc-9213-targeted-http-cache-control)

-   `CDN-Cache-Control` Field のこと。
-   特定のキャッシュ(主に CDN)に対してだけ有効にしたい `Cache-Control` の名指し版。
-   Fastly でいう `Surrogate-Control` の標準仕様。

### [RFC 9218: Extensible Prioritization Scheme for HTTP](#rfc-9218-extensible-prioritization-scheme-for-http)

-   HTTP/2 には優先度制御が最初から含まれていた。
-   でも難しすぎて誰も使いこなせなかった。
-   そっちは deprecate してもう少しシンプルで使いやすくしよう。

### [RFC 9220: Bootstrapping WebSockets with HTTP/3](#rfc-9220-bootstrapping-websockets-with-http3)

-   WebSocket を HTTP/3 の上で流すためのバインディング的な仕様。
-   開発者はあまり気にするところではない。

### [RFC 9230: Oblivious DNS over HTTPS](#rfc-9230-oblivious-dns-over-https)
...
## [HTTP RFC Publication Study](#http-rfc-publication-study)
...
-   HTTP RFC Publication Study
    -   [https://web-study.connpass.com/event/250730/](https://web-study.connpass.com/event/250730/)

アーカイブは以下に公開している。このあたりの話が資料付きで解説されているのでそちらで補足してほしい。

-   HTTP RFC Publication Study - YouTube
    -   [https://www.youtube.com/watch?v=\_hfG0HCufbs](https://www.youtube.com/watch?v=_hfG0HCufbs)
