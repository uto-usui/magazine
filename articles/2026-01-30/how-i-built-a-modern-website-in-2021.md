...
source: "https://kentcdodds.com/blog/how-i-built-a-modern-website-in-2021"
publishedDate: "2021-09-03"
category: "frontend"
feedName: "Kent C. Dodds"
---

For over half of 2021, I worked on a complete rewrite of kentcdodds.com. You're reading this on the rewrite of this site! Are you using dark mode or light mode? Have you [signed in](https://kentcdodds.com/login) and selected your team yet? Have you tried to call into [the Call Kent Podcast](https://kentcdodds.com/calls)? This blog post isn't about these and other features of the new site, but how I built it. There's too much to dive deep in a single blog post. I just want to give you an overview of the technologies and libraries I used to make this experience for you.

If you haven't read it already, get a higher-level overview of what this website can do for your learning and advancement as a software engineer by reading [Introducing the new kentcdodds.com](https://kentcdodds.com/blog/introducing-the-new-kentcdodds.com) first.

I've migrated from Postgres/Redis to SQLite. Read about that in the post [I Migrated from a Postgres Cluster to Distributed SQLite with LiteFS](https://kentcdodds.com/blog/i-migrated-from-a-postgres-cluster-to-distributed-sqlite-with-litefs).

## [Context](#context)

Before we get too far into this I want to make my one thing clear. If this was a simple developer's blogfolio site, my tech choices could be labeled as over-engineering and I would agree. However, I wanted to build an above and beyond experience, so I had to make thoughtful architectural decisions. What I'm doing on this site could definitely _not_ be done with wordpress and a CDN 😆

If you're a beginner looking for how to build your personal website, this blog post is definitely not the place to learn that. If I were to build a simple personal website, I would still use [Remix.run](https://remix.run/), but I would probably have it running on netlify serverless functions and write the content as markdown which has built-in support in Remix. That would be drastically simpler.
...
At the time of this writing (October 2021) here are the [`cloc`](https://github.com/AlDanial/cloc) stats:
...
And adding up the amount of content on the four seasons of [the Chats with Kent Podcast](https://kentcdodds.com/chats) comes to about 35 hours of content + the ever-growing 3 hours of content on the brand new [Call Kent Podcast](https://kentcdodds.com/calls) which incidentally is also more than it takes to listen to Jim Dale read the first three Harry Potter books (unless you listen at 3x [like me](https://kentcdodds.com/about) #subtlebrag 🙃).
...
I didn't do this whole thing by myself. You can check out [the credits page](https://kentcdodds.com/credits) for details on contributors. I was the primary code contributor and I made all the architecture decisions (and mistakes? only time will tell 😅) for the site.

The first commit was in [November 2020](https://github.com/kentcdodds/kentcdodds.com/commit/4fefae1389a7c3faea2fc2b321d0953a80d0e414). Most development has taken place in the last 4-5 months. There are ~945 commits so far 🤯.

[![GitHub contributor graph](https://res.cloudinary.com/kentcdodds-com/image/upload/w_1135,q_auto,f_auto,b_rgb:e6e9ee/kentcdodds.com/content/blog/how-i-built-a-modern-website-in-2021/contributor-graph-light)](https://github.com/kentcdodds/kentcdodds.com/graphs/contributors)
...
[![Excalidraw diagram of a deployment pipeline](https://res.cloudinary.com/kentcdodds-com/image/upload/w_1135,q_auto,f_auto,b_rgb:e6e9ee/kentcdodds.com/content/blog/how-i-built-a-modern-website-in-2021/deployment-pipeline-light)](https://res.cloudinary.com/kentcdodds-com/image/upload/kentcdodds.com/content/blog/how-i-built-a-modern-website-in-2021/deployment-pipeline-light)
...
#### [GitHub Actions: 🥬 Refresh Content](#github-actions--refresh-content)
...
[![Excalidraw diagram of a databases in different regions](https://res.cloudinary.com/kentcdodds-com/image/upload/w_1135,q_auto,f_auto,b_rgb:e6e9ee/kentcdodds.com/content/blog/how-i-built-a-modern-website-in-2021/database-connectivity-light)](https://res.cloudinary.com/kentcdodds-com/image/upload/kentcdodds.com/content/blog/how-i-built-a-modern-website-in-2021/database-connectivity-light)
...
### [Fly Request Replays](#fly-request-replays)

[![Excalidraw diagram of a user POST request made to different regions being routed to the primary region](https://res.cloudinary.com/kentcdodds-com/image/upload/w_1135,q_auto,f_auto,b_rgb:e6e9ee/kentcdodds.com/content/blog/how-i-built-a-modern-website-in-2021/fly-request-replays-light)](https://res.cloudinary.com/kentcdodds-com/image/upload/kentcdodds.com/content/blog/how-i-built-a-modern-website-in-2021/fly-request-replays-light)
...
## [Local Development with MSW](#local-development-with-msw)
...
And because I have this set up with MSW to work locally, I can make my E2E tests use the same thing and stay resilient. If I want to run my E2E tests against the real APIs, then all I have to do is _not_ `--require ./mocks` and everything's hitting real APIs.

MSW is an enormous productivity and confidence booster for me.

## [Caching with Redis/LRU](#caching-with-redislru)
...
-   `forceFresh` allows you to skip looking at the cache and will call `getFreshValue` even if the value hasn't expired yet. If you provide a string then it splits that string by `,` and checks whether the `key` is included in that string. If it is then we'll call `getFreshValue`. This is useful for when you're calling a cachified function which calls other cachified functions (like the function that retrieves all the blog mdx files). You can call that function and only refresh _some_ of the cache values, not all of them.
...
## [Image optimization with Cloudinary](#image-optimization-with-cloudinary)
...
## [MDX Compilation with mdx-bundler](#mdx-compilation-with-mdx-bundler)
...
No such bundler existed, so I made one: [`mdx-bundler`](https://github.com/kentcdodds/mdx-bundler). I started with rollup and then gave esbuild a try and was blown away. It's out-of-this-world fast (though still not fast enough to bundle-on-demand so I do cache the compiled version).
...
Now _that's_ what I'm talking about! And with Remix, I can easily query directly in my `loader`, and then have that **typed** data available in my component:
...
## [Authentication with Magic Links](#authentication-with-magic-links)

A while back, I tweeted some words that I'm eating...

> Here are the only reasonable options for authentication in apps: 1. Use a cloud provider for auth 2. Have a team dedicated to auth for the company's apps 3. Use HTTP basic auth because security clearly doesn't matter that much for you anyway 🤷‍♂️

[40](https://x.com/kentcdodds/status/1299758509312024576) [](https://x.com/intent/retweet?tweet_id=1299758509312024576)[480](https://x.com/intent/like?tweet_id=1299758509312024576)[](https://x.com/kentcdodds/status/1299758509312024576)
...
[![Excalidraw diagram of a user going to the login screen, clicking the magic link, and getting authenticated](https://res.cloudinary.com/kentcdodds-com/image/upload/w_1135,q_auto,f_auto,b_rgb:e6e9ee/kentcdodds.com/content/blog/how-i-built-a-modern-website-in-2021/auth-flow-light)](https://res.cloudinary.com/kentcdodds-com/image/upload/kentcdodds.com/content/blog/how-i-built-a-modern-website-in-2021/auth-flow-light)
...
[![Excalidraw diagram of a user going to an authenticated page and the session being resolved to a user or the user getting redirected to login](https://res.cloudinary.com/kentcdodds-com/image/upload/w_1135,q_auto,f_auto,b_rgb:e6e9ee/kentcdodds.com/content/blog/how-i-built-a-modern-website-in-2021/session-flow-light)](https://res.cloudinary.com/kentcdodds-com/image/upload/kentcdodds.com/content/blog/how-i-built-a-modern-website-in-2021/session-flow-light)
...
![Remix logo](https://res.cloudinary.com/kentcdodds-com/image/upload/f_auto,q_auto,dpr_2.0,w_1600/kentcdodds.com/content/blog/how-i-built-a-modern-website-in-2021/remix)
...
3.  The ability to have CSS for a specific route and know that I won't clash with CSS on _any other route_. 👋 goodbye CSS-in-JS.
