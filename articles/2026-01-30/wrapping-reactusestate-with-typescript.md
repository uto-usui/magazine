...
source: "https://kentcdodds.com/blog/wrapping-react-use-state-with-type-script"
...
This works great, and powers the "dark mode" support for all the [Epic React](https://epicreact.dev/) workshop apps (for example [React Fundamentals](https://react-fundamentals.netlify.app/)).
...
For us reading the code we know that `setMode` is a callable function, so the question is: why is the `setMode` type both a function _and_ a string?
...
This will help us when we call `setMode` to ensure we not only call it with a string, but the _right_ type of string. I also created type aliases for this and the dispatch function to make the prop types easier as I pass these values around my app.
