...
source: "https://kentcdodds.com/blog/improve-test-error-messages-of-your-abstractions"
...
([Yes, that is a test](https://kentcdodds.com/blog/but-really-what-is-a-javascript-test)).
...
![visual output of the above](https://res.cloudinary.com/kentcdodds-com/image/upload/f_auto,q_auto,dpr_2.0,w_1600/v1625032930/kentcdodds.com/content/blog/improve-test-error-messages-of-your-abstractions/one.png)
...
Please keep in mind, I am _not_ recommending you create useless abstractions like the one above. As with everything, you should be applying [AHA Programming](https://kentcdodds.com/blog/aha-programming) (and [for testing](https://kentcdodds.com/blog/aha-testing)). This blog post is just useful for situations where the abstraction is clear and you want to include assertions in your abstraction.
...
What!? That's not _nearly_ as helpful! What if we had a bunch of places we're calling `assertAdd`? What good is that codeframe going to do us? How do we know which one failed. Oh, there it is, I we _do_ get a line in the stack trace, but... like... talk about a step backward. I'd much rather have the line that called `assertAdd` be what shows up in the codeframe.

Well, there's no API into Jest for this (yet?), but you _can_ force Jest to give you a codeframe where you want. So what I'm going to show you next is how we can make this error output like this:
...
Ok, so you'll notice that the stack trace we've got here is very different from the one that Jest shows us. This is because most of the stuff in there is pretty useless to developers. It's just noise. Why do developers need to know that their code ran through `mapper` function at `queueRunner.js:30:19`? Yeah, pretty useless. So [when Jest formats the stack trace](https://github.com/facebook/jest/blob/7e37b0ff06de590efd6678db30e0ca73969dc2df/packages/jest-message-util/src/index.ts#L274-L316), it [filters out a bunch of the noise](https://github.com/facebook/jest/blob/7e37b0ff06de590efd6678db30e0ca73969dc2df/packages/jest-message-util/src/index.ts#L280), and we're left with:
...
Definitely more helpful. The next thing Jest does is [it takes the first line](https://github.com/facebook/jest/blob/7e37b0ff06de590efd6678db30e0ca73969dc2df/packages/jest-message-util/src/index.ts#L287) in the remaining stack trace lines and [creates the codeframe for the first line](https://github.com/facebook/jest/blob/7e37b0ff06de590efd6678db30e0ca73969dc2df/packages/jest-message-util/src/index.ts#L297). Then it [formats filepaths](https://github.com/facebook/jest/blob/7e37b0ff06de590efd6678db30e0ca73969dc2df/packages/jest-message-util/src/index.ts#L309) and we're left with the relatively useless error + codeframe + stack trace shown above.
...
![visual representation of the good output](https://res.cloudinary.com/kentcdodds-com/image/upload/f_auto,q_auto,dpr_2.0,w_1600/v1625032930/kentcdodds.com/content/blog/improve-test-error-messages-of-your-abstractions/two.png)

The problem with this is we actually don't want to _just_ filter out our utility. What if that utility function is built on top of other functions. So really, we want to remove _everything_ above our utility as well. This is actually what Jest's `expect` does and [it uses `Error.captureStackTrace`](https://github.com/facebook/jest/blob/7e37b0ff06de590efd6678db30e0ca73969dc2df/packages/expect/src/index.ts#L314).
...
Wow, that's a lot cleaner. So we pass the `error` we want updated and the function we want removed from the stack trace. That argument is called the `constructorOpt`. [According to the Node.js docs](https://nodejs.org/api/errors.html#errors_error_capturestacktrace_targetobject_constructoropt):
...
![visual representation of the error message](https://res.cloudinary.com/kentcdodds-com/image/upload/f_auto,q_auto,dpr_2.0,w_1600/v1625032930/kentcdodds.com/content/blog/improve-test-error-messages-of-your-abstractions/two.png)
...
But manipulating the stack trace for improved error messages can be good knowledge to have, even for things you publish to a registry. For example, [DOM Testing Library does this in `waitFor`](https://github.com/testing-library/dom-testing-library/blob/8846eaf20972f8e41ed11f278948ac38a692c3f1/src/wait-for.js#L79) to make sure failures of asynchronous utilities (like `find*` queries and `waitFor` itself) have beautiful errors and sensible stack traces (async stack traces are pretty useless).
