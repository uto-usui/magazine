...
source: "https://kentcdodds.com/blog/how-to-test-custom-react-hooks"
...
Let's say we've got this custom hook called `useUndo` (inspired by [`useUndo`](https://github.com/xxhomey19/use-undo) by [Homer Chen](https://x.com/xxhomey19)):
...
Let's say we want to write a test for this so we can maintain confidence that as we make changes and bug fixes we don't break existing functionality. To get the maximum confidence we need, we should ensure that our tests [resemble the way the software will be used](https://x.com/kentcdodds/status/977018512689455106). Remember that software is all about automating things that we don't want to or cannot do manually. Tests are no different, so consider how you would test this manually, then write your test to do the same thing.

A mistake that I see a lot of people make is thinking "well, it's just a function right, that's what we love about hooks. So can't I just call the function and assert on the output? Unit tests FTW!" They're not wrong. It _is_ just a function, but technically speaking, it's not a [_pure_ function](https://en.wikipedia.org/wiki/Pure_function) (your hooks are supposed to be [idempotent](https://en.wikipedia.org/wiki/Idempotence) though). If the function were _pure_, then it would be a simple task of calling it and asserting on the output.

If you try simply calling the function in a test, you're breaking [the rules of hooks](https://reactjs.org/docs/hooks-rules.html) and you'll get this error:
...
Now, you _might_ start to think: "Hey, if I just mock the built-in React hooks I'm using like `useState` and `useEffect` then I could still test it like a function." But for the love of all things pure, please don't do that. You throw away a LOT of confidence in doing so.

But don't fret, if you were to test this manually, rather simply calling the function, you'd probably write a component that uses the hook, and then interact with that component rendered to the page (perhaps using [storybook](https://storybook.js.org/)). So let's do that instead:
...
Now, having those example components is probably a good idea anyway (they're great for [storybook](https://storybook.js.org/) for example), but sometimes it can be nice to create a little helper that doesn't actually have any UI associated with it and you interact with the hook return value directly.
...
This is why `renderHook` from [`@testing-library/react`](https://github.com/testing-library/react-testing-library) exists. Here's what this test would be like if we use `@testing-library/react`:
...
Note, you _can_ test more than a single hook by simply calling all the hooks you want in the callback function you pass to `renderHook`.
