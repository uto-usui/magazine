...
source: "https://blog.jxck.io/entries/2016-03-14/web-font-noto-sans.html"
...
加えて、このサイトはあくまで _技術ブログ_ なので、さらに使用する文字は限定される。
...
## [Noto Sans Fonts](#noto-sans-fonts)
...
> Note: Noto Sans JP has been subsetted to 6,992 characters and contains most of the characters defined by JIS X 0208 and some additional characters from Shift\_JIS. The set includes 6,390 Kanji as well as 602 non Kanji characters such as Hiragana, Katakana (including half-width variants), ASCII and full-width and punctuations.
> 
> — [https://www.google.com/fonts/earlyaccess#Noto+Sans+JP](https://www.google.com/fonts/earlyaccess#Noto+Sans+JP)
...
本サイトでは、Markdown から生成していることもあり、「通常の文字」と「_太文字_」しか基本的に使わない。
...
## [JIS X 0208 第一水準](#jis-x-0208-第一水準)
...
しかし、このブログは _技術ブログ_ であり、使われる漢字は非常に限定的だ。

そこで、必要な漢字のみを収録したサブセットを作成する。

## [基本ラテン文字 (0000-0070)](#基本ラテン文字-0000-0070)
...
## [CJK 記号と句読点(3000-303F)](#cjk-記号と句読点3000-303f)
...
## [半角形と全角形(FF00-FFE0)](#半角形と全角形ff00-ffe0)

全角の英数や半角カタカナなど。しかし、このブログで _これらは使わない_。
...
### [常用漢字と第一水準漢字の差分](#常用漢字と第一水準漢字の差分)

気になって、二つのセットの diff を取ってみたら、「 _JIS 第一水準にはないが、常用漢字にある漢字_ 」があることがわかった。
...
他にも、常用漢字一覧には、まだまだ「 _技術ブログではおおよそ出てこなさそうな漢字_ 」がいくつか見られる。
...
### [font-family の指定](#font-family-の指定)
