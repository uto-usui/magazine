...
source: "https://kentcdodds.com/blog/listify-a-java-script-array"
...
Have you heard of [`Intl.ListFormat`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/ListFormat)? Well, it's likely that this can do all you need and more. Let's take a look:
...
Sojourner, Opportunity, Spirit, Curiosity, and Perseverance

**`disjunction`**

Sojourner, Opportunity, Spirit, Curiosity, or Perseverance
...
There's also a lesser-known `localeMatcher` option which can be configured to either `'lookup'` or `'best fit'` (defaults to `'best fit'`). As far as I can tell from [mdn](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl#locale_negotiation), its purpose is to tell the browser how to determine which locale to use based on the one given in the constructor. In my own testing I was unable to determine a difference in functionality switching between these options 🤷‍♂️
...
So I put together a little function that suited my use case pretty well, and I want to share it with you. Before I do, I want to be clear that I _did_ try this without reduce (using a for loop) and I think the reduce method was considerably simpler. Feel free to take a whack at the for loop version though if you want.
...
So it just goes to show you that you _might_ be doing some extra work that you might not need to be doing. The platform probably does this for you automatically! Oh, and just for fun, here's a TypeScript version of that finished code (I'm in the process of migrating this project to TypeScript).
...
Oh, by the way, you'll always want to double-check browser support for whatever you're using. There's [caniuse.com](https://caniuse.com/?search=listformat) and [the MDN article on Intl.ListFormat](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/ListFormat) also has [a chart](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/ListFormat#browser_compatibility).
