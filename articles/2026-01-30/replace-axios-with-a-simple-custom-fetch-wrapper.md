...
I remember being with [Matt Zabriskie](https://x.com/mzabriskie) when he hatched the idea of a vanilla JavaScript version of AngularJS's `$http` service. It seemed like a brilliant idea and that night in his hotel room at MidwestJS, he put together the first iteration.

It was awesome because working with raw [XMLHttpRequest](https://developer.mozilla.org/en-US/docs/Web/API/XMLHttpRequest) to make HTTP requests was not very fun. His library, which he later called [`axios`](https://github.com/axios/axios) is a brilliant work and functioned both in NodeJS and the Browser which I remember him being really excited about (and I was too).

It's been almost six years now and if you're reading this chances are you've at least heard of it and very likely used it in the past or are using it now. It has an enormous and growing number of [downloads on npm](https://www.npmtrends.com/axios). And while Matt's long moved on from the project, it is still actively maintained.

Since it was released, the browser standard has evolved to add a new, promise-based API for making HTTP requests that provided a much nicer developer experience. This API is called [fetch](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API) and if you haven't used it yet, you really ought to check it out. It's widely supported and easily polyfillable (my favorite is [unfetch](https://github.com/developit/unfetch) because the dog mascot is cute 🐶).
...
I have a fetch wrapper for [my bookshelf app](https://github.com/kentcdodds/bookshelf) which has served me well. Let's build it together:
...
However, the built-in `window.fetch` API doesn't handle errors in the same way `axios` does. By default, `window.fetch` will only reject a promise if the actual request itself failed (network error), not if it returned a "Client error response". Luckily, the `Response` object has [an `ok`](https://developer.mozilla.org/en-US/docs/Web/API/Response/ok) property which we can use to reject the promise in our wrapper:
...
The next thing we want to do is be able to send data to the backend. We _can_ do this with our current API, but let's make it easier:
...
So basically if we have a token in `localStorage` by that key, then we add the `Authorization` header (per [the JWT spec](https://jwt.io/)) which our server can then use to determine whether the user is authorized. Very common practice there.
