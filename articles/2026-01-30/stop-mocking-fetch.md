...
[msw](https://github.com/mswjs/msw) is short for "Mock Service Worker". Now, service workers don't work in Node, they're a browser feature. However, `msw` supports Node anyway for testing purposes.

The basic idea is this: create a mock server that intercepts all requests and handle it just like you would if it were a real server. In my own implementation, this means I make a "database" either out of `json` files to "seed" the database, or "builders" using something like [faker](https://www.npmjs.com/package/faker) or [test-data-bot](https://www.npmjs.com/package/@jackfranklin/test-data-bot). Then I make server handlers (similar to the express API) and interact with that mock database. This makes my tests fast and easy to write (once you have things set up).

You may have used something like [`nock`](https://github.com/nock/nock) to do this sort of thing before. But the cool thing about `msw` (and something I may write about later), is that you can also use all the exact same "server handlers" in the browser during development as well. This has a few great benefits:

1.  If the endpoint isn't ready
2.  If the endpoint is broken
3.  If your internet connection is slow or non-existent

You might have heard of [Mirage](https://miragejs.com/) which does much of the same thing. However (currently) mirage does not use a service worker in the client and I really like that the network tab works the same whether I have msw installed or not. [Learn more about their differences](https://miragejs.com/docs/comparison-with-other-tools/#msw).
...
One reasonable concern about this approach is that you end up putting all of your server handlers in one place and then the tests that rely on those server handlers end up in entirely different files, so you lose the benefits of [colocation](https://kentcdodds.com/blog/colocation).

First off, I'd say that you want to only colocate the things that are _important and unique_ to your test. You wouldn't want to have to duplicate all the setup in every test. Only the parts that are unique. So the "happy path" stuff is typically better to just include in your setup file, removed from the test itself. Otherwise you have too much noise and it's hard to isolate what's actually being tested.
...
There's definitely more to do with `msw`, but let's just wrap up for now. If you want to see `msw` in action, my 4 part workshop "Build React Apps" (included in [EpicReact.Dev](https://epicreact.dev/)) uses it and you can [find all the material on GitHub](https://github.com/kentcdodds/bookshelf).
...
> Spent the day refactoring a React component. Well written tests with react-testing-library (thanks [@kentcdodds](https://x.com/kentcdodds)) gave me great confidence and helped me catch a subtitle error. Take-away: good unit tests are so important!

[1](https://x.com/d11erh/status/1269440558638018561) [](https://x.com/intent/retweet?tweet_id=1269440558638018561)[34](https://x.com/intent/like?tweet_id=1269440558638018561)[](https://x.com/d11erh/status/1269440558638018561)

[7](https://x.com/kentcdodds/status/1269442973525094400) [](https://x.com/intent/retweet?tweet_id=1269442973525094400)[146](https://x.com/intent/like?tweet_id=1269442973525094400)[](https://x.com/kentcdodds/status/1269442973525094400)
