...
source: "https://zenn.dev/uhyo/articles/state-update-while-rendering"
...
> Cannot update a component (`Parent`) while rendering a different component (`Child`). To locate the bad setState() call inside `Child`, follow the stack trace as described in [https://react.dev/link/setstate-in-render](https://react.dev/link/setstate-in-render)
...
Reactにおいて、関数コンポーネントの呼び出しが1回増える程度のことでパフォーマンスに与える影響は微々たるものです。それを前提に、Reactは宣言的にUIを記述できる仕組みとUXの最適化を両立させているのです。また、前述のようにこの機能は自分自身のステートを更新することしかできません。これもパフォーマンスへの悪影響を最小限に留めるための制約です。さすがにコンポーネントツリー全体をむやみに複数回レンダリングするのは無駄が大きいですが、特定の関数コンポーネントひとつだけならば影響は抑えられます。やり直しの場合には子コンポーネントのレンダリングをスキップするのも、必要ならば複数回関数を呼び出すけど無駄にパフォーマンスを落とすことはしないというバランスを取った設計と言えます。
...
`prevItems`という名前だと「状態の変化を検知する」というある種手続き的なニュアンスが強くなります。そもそも、「前回のレンダリング」という概念を意識しなければならないのが微妙です。宣言的UIなのですから、コンポーネントはあくまで「現時点」でのUIを計算することに集中したいですね。

その点、`currentItems`という名前にすれば自己補正という考え方に合った命名になります。「現時点の認識」をベースに`currentItems`と`items`の不整合を検知するという考え方ができるので、より宣言的です。

それにも関わらず公式ドキュメントで`prevItems`という名前が使われているのは、「状態の変化を検知する」というメンタルモデルのほうが多くのReact利用者にとっては理解しやすいからかもしれません。Reactの公式ドキュメントは上級者にとっても学びがある素晴らしいものですが、ターゲット層を広く想定して完璧さよりも受け入れやすさが優先されることもあります。
...
しかし、上記のようにした場合、「ステートを更新したこと」よりも「エラーが投げられたこと」が優先されてしまいます。つまり、レンダリングの結果は「ステート更新してやり直し」ではなく「エラーが発生」になってしまいます（エラーがエラーバウンダリーに捕捉されます）。したがって、レンダリングのやり直しを指示した場合でも、返り値をちゃんと返す必要があります。
...
このように、フックを全部過ぎてからnullを返すのは大丈夫です。レンダリングのやり直しを指示した時点でListの返り値のJSXは使われませんので、nullを返しても問題ありません（もちろん、画面に一瞬だけnullが反映されることもありません）。
...
このように、コンポーネントには自己補正機能を持たせることができます。このことを知っておくと、コンポーネント設計の幅が広がるでしょう。