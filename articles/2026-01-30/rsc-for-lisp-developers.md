...
source: "https://overreacted.io/rsc-for-lisp-developers/"
...
Of course, I _could_ evaluate it later:
...
React Server Components (RSC) is a client-server programming paradigm that uses a similar idea to refer to client code from the server code. The `'use client'` directive lets you [import code designed for the client—but without running it](https://overreacted.io/why-does-rsc-integrate-with-a-bundler/#serializing-modules):
...
Like quoting, it marks a piece of code to be treated as data. _Unlike_ quoting in LISP, the result you get back is opaque—you can’t transform or introspect that code.

This means that whoever imports `onClick` from the backend code won’t get an actual `onClick` function—instead, they’ll get `'/js/chunk123.js#onClick'` or something like that identifying _how to load_ this module. It gives you code-as-data. Unlike with LISP quoting, this is [implemented at the compile time](https://overreacted.io/why-does-rsc-integrate-with-a-bundler/#rsc-bundler-bindings) using a bundler.
...
What this gives us is an ability to write a program that composes behaviors that execute at different stages (on the server and the client) in a very modular way. [See here for example.](https://overreacted.io/impossible-components/#final-code) The parts outside the “quote” deal with server-only resources, while the parts inside the “quote” are stateful and exist on the client—but they are composed. The server stuff can wrap the client stuff, the client stuff can wrap the server stuff, as long as you’re doing all composition from the server. And what doing composition on the server enables is a guarantee that all the server stuff runs [within a single request/response roundtrip](https://overreacted.io/one-roundtrip-per-navigation/). It’s also [progressively streamed](https://overreacted.io/progressive-json/).
