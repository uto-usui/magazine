...
source: "https://jakearchibald.com/2024/how-should-selectedoption-work/"
...
## [A brief intro to customisable `<select>`](#a-brief-intro-to-customisable-select)
...
## [What about `<selectedoption>`?](#what-about-selectedoption)
...
This is kinda new and weird behaviour. [I recently wrote that I didn't like elements that modify themselves](https://jakearchibald.com/2024/attributes-vs-properties/#attributes-should-be-for-configuration), as I think the light DOM should have a single owner. But, I can't think of a better way to do this, given:
...
## [But what if the selected `<option>` is modified?](#but-what-if-the-selected-option-is-modified)
...
### [Option: Nothing by default, but provide a way to trigger an update](#option-nothing-by-default-but-provide-a-way-to-trigger-an-update)
...
### [Option: Automatically reset the content when anything in the selected `<option>` changes](#option-automatically-reset-the-content-when-anything-in-the-selected-option-changes)
...
### [Option: Automatically reset the content when anything in the selected `<option>` changes… debounced](#option-automatically-reset-the-content-when-anything-in-the-selected-option-changes-debounced)
...
### [Option: Perform targeted DOM changes when something in the selected `<option>` changes](#option-perform-targeted-dom-changes-when-something-in-the-selected-option-changes)
