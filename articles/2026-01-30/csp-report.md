---
title: "CSP Report 収集と実レポートの考察"
source: "https://blog.jxck.io/entries/2017-02-13/csp-report-case-study.html"
...
しかし、本質的な CSP の責務は「_ポリシーに照らし合わせた違反のブロック_」であり、その「_ポリシーの違反が攻撃とは限らない_」という点には注意が必要だ。
...
なにより重要なのは、_このサイトはほとんど全てが静的に生成されており、かつ `<input>` フォームが無い_ という点である。
...
以下が CSP をデプロイしてから、今月までのレポート発生のグラフである。
  ...
  ![report-uri.io で生成した過去 12 ヵ月の CSP レポートのグラフ](https://blog.jxck.io/entries/2017-02-13/graph-of-csp-report-12month.png?200901_002432 "graph of csp report 12 month")
...
## [Report Case Study](#report-case-study)

次に個々のレポートを細かく見た結果、よく発生していたものについて紹介する。

### [DOM Change by User](#dom-change-by-user)
...
### [Browser Inline Style](#browser-inline-style)
...
このとき、HTML の中にある Inline Style がポリシー違反と判定されるため、レポートが上がる。
  ...
  ![Chrome でテキストを表示すると、整形のために埋め込まれた HTML が CSP 違反を起こす](https://blog.jxck.io/entries/2017-02-13/chrome-inline-style-violation.png?200901_002432 "inline style violation for RSS feed in chrome")
...
### [Firefox view:source](#firefox-viewsource)
...
Chrome でも同じくオリジンが変わるが、ここでレポートはあがらない実装になっているようだ。
  ...
  ![Firefox の view-source:// でソースを表示すると、整形のために埋め込まれた HTML が CSP 違反を起こす](https://blog.jxck.io/entries/2017-02-13/firefox-view-source-violation.png?200901_002432 "view-source:// violates csp policy in firefox")

これは対応せず無視することとした。

### [about:blank\_](#aboutblank_)
