...
source: "https://kentcdodds.com/blog/i-migrated-from-a-postgres-cluster-to-distributed-sqlite-with-litefs"
publishedDate: "2022-11-01"
category: "frontend"
feedName: "Kent C. Dodds"
---

Over the last few months, I've been heads-down on building the content for [EpicWeb.dev](https://epicweb.dev/). And I've been building it all in the open on [my YouTube channel](https://kcd.im/YouTube). If you've been watching [my EpicWeb.dev live streams](https://www.youtube.com/playlist?list=PLV5CVI1eNcJhNHoK946zgZ8ZivZbo6uyx), you'll know that I've been building the app we'll use to learn web dev: Rocket Rental 🚀. With that app, I've started with SQLite and I've been trying to decide whether I should move over to Postgres.

When [I released the 2021 update to kentcdodds.com](https://kentcdodds.com/blog/how-i-built-a-modern-website-in-2021), I decided to use Postgres on [Fly.io](https://fly.io/) because of its support for multi-regional deploys and both Postgres and Redis clusters with automatic data replication (I use Redis for caching slow/rate limited third party APIs and expensive computations). That proved to work out really well. My site is fast wherever you are in the world. However, there are a few downsides:
...
Several months ago, I met [Kurt](https://x.com/mrkurt) (Fly.io CEO) at [Remix Conf](https://remix.run/conf). We talked about a new project he's been very excited about that I'd heard rumblings about called [Litestream](https://litestream.io/). He suggested that for many categories of applications, SQLite can provide an even better user experience than Postgres along with a simplified developer experience. Whenever I hear "better UX _and_ DX," I'm interested.

Litestream was [originally created](https://litestream.io/blog/why-i-built-litestream/) to improve reliability of SQLite in edge cases (specifically, disaster recovery), but the way this was done also lends itself well to the idea of distributed SQLite deployments.

Turns out by the time Kurt and I had our conversation, he'd already hired [Ben Johnson](https://x.com/benbjohnson) (the author of Litestream) to work at Fly. Ben wrote [an awesome article](https://fly.io/blog/all-in-on-sqlite-litestream/) explaining what Litestream is. I recommend you give it a read if you're unconvinced that SQLite is production-ready. He'll convince you it is.

## [LiteFS](#litefs)

Litestream has had "read-replicas" as a desired future feature for over a year. Ben joined Fly to work on that and has now released [LiteFS](https://fly.io/blog/introducing-litefs). Architecturally, LiteFS works similarly to Postgres clusters. One LiteFS node is the "primary" and other nodes automatically replicate data writes. LiteFS acts as a proxy to your SQLite database (you connect to LiteFS instead of the underlying database) and then LiteFS effectively replays data updates to all read replicas. And this reply happens behind the scenes normally within 200ms (on par with distributed Postgres clusters).
...
Additionally, SQLite data access is much faster than Postgres which is "fast enough" normally, but actually means [SQLite suffers much less from N+1 queries](https://www.sqlite.org/np1queryprob.html) (which should be avoided, but this is nice).
...
## [Evaluating the existing data](#evaluating-the-existing-data)
...
## [Updating App code](#updating-app-code)
...
There really aren't many situations where a mutation on a GET makes sense so this works great. All the user experiences is a _slightly_ longer response time in these rare scenarios.
...
### [The double-thick wall](#the-double-thick-wall)
...
### [Deploying LiteFS](#deploying-litefs)
...
2.  Double-check volumes in `kcd` (`dfw` is primary)
...
There's more interesting stuff that happened here. If you're interested in watching the migration, then tune in [here](https://www.youtube.com/watch?v=1kWcU3d5P00).
...
"Went off pretty much without issue" that is... Until I realized I had a hard core memory leak... Read more about that in [Fixing a Memory Leak in a Production Node.js App](https://kentcdodds.com/blog/fixing-a-memory-leak-in-a-production-node-js-app).
