---
title: "Pure Modules"
source: "https://kentcdodds.com/blog/pure-modules"
publishedDate: "2018-04-01"
category: "frontend"
feedName: "Kent C. Dodds"
---

A few weeks ago, I saw [this tweet](https://x.com/RReverser/status/984854567706877952) from [Ingvar Stepanyan](https://x.com/RReverser):

𝕏 post data not available: [https://x.com/RReverser/status/984854567706877952](https://x.com/RReverser/status/984854567706877952)

Followed by [this one](https://x.com/RReverser/status/984854857050882049):

𝕏 post data not available: [https://x.com/RReverser/status/984854857050882049](https://x.com/RReverser/status/984854857050882049)
...
This made me think of something else that I feel is important and I'd like to share with you. I [quote tweeted Ingvar's tweet](https://x.com/kentcdodds/status/985192759165251584):

> I totally agree. Just because that's not the way the spec is didn't mean you can't do that yourself. I highly recommend you make your modules export functions that do things rather than do things on import. [twitter.com/RReverser/status/984854567706877952](https://twitter.com/RReverser/status/984854567706877952)

[0](https://x.com/kentcdodds/status/985192759165251584) [](https://x.com/intent/retweet?tweet_id=985192759165251584)[27](https://x.com/intent/like?tweet_id=985192759165251584)[](https://x.com/kentcdodds/status/985192759165251584)

Ingvar [expanded on what I mean](https://x.com/RReverser/status/985286856681054208) (unknowingly I'm sure) in another thread (sadly this thread has now been deleted).

### [Why pure modules?](#why-pure-modules)
...
I expect this code would work in production as expected. There are a few problems I have with code like this (or in general any impure modules). Before we move on it's important to realize that before the `console.log('ready')` line is run, all the code in `b.js`and `c.js` has been run first.
...
**As a related (and very important) part of this, you cannot [treeshake](https://developer.mozilla.org/en-US/docs/Glossary/Tree_shaking) that unused code!**

### [Inability to choose the order of operations 😡](#inability-to-choose-the-order-of-operations-)
...
### [Impact on testing 😵](#impact-on-testing)

It's a pretty widely accepted fact that it's easier to test pure functions than impure functions. The same applies with modules. What if I wanted to test the `b.js` module? I'd have to initialize the DOM before importing `b.js` so `c.js`can be initialized properly, but then how do I test it again? I have to do weird things with the module system to re-import those modules again after initializing the DOM differently.

With jest, you have [`jest.resetModules()`](https://facebook.github.io/jest/docs/en/jest-object.html#jestresetmodules) which makes this much easier, but it's still not super simple, nor is it straightforward for anyone maintaining those tests.
...
-   **Impact on testing:** We could easily run the `init`function from `b.js` in a test as many times, re-initializing the DOM before each test with exactly what we need without any trouble or hacks. 🎉
