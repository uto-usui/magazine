...
## [Don’t Get Distracted by Imaginary Problems](#dont-get-distracted-by-imaginary-problems)
...
## [Marie Kondo Your Lint Config](#marie-kondo-your-lint-config)
...
## [Writing Resilient Components](#writing-resilient-components)
...
## [Principle 1: Don’t Stop the Data Flow](#principle-1-dont-stop-the-data-flow)

### [Don’t Stop the Data Flow in Rendering](#dont-stop-the-data-flow-in-rendering)
...
### [Don’t Stop the Data Flow in Side Effects](#dont-stop-the-data-flow-in-side-effects)
...
However, one _could_ design an API that _can_ be statically analyzed for consistency. The [React `useEffect` Hook](https://overreacted.io/a-complete-guide-to-useeffect/) is an example of such API:
...
![Demo of exhaustive-deps lint rule](https://overreacted.io/writing-resilient-components/useeffect.gif)
...
**The `useEffect` API flips the default by encouraging consistency.** This [might feel unfamiliar at first](https://overreacted.io/a-complete-guide-to-useeffect/), but as a result your component becomes more resilient to changes in the logic. And since the “dependencies” are now explicit, we can _verify_ the effect is consistent using a lint rule. We’re using a linter to catch bugs!

* * *

### [Don’t Stop the Data Flow in Optimizations](#dont-stop-the-data-flow-in-optimizations)
...
## [Principle 2: Always Be Ready to Render](#principle-2-always-be-ready-to-render)
...
## [Principle 3: No Component Is a Singleton](#principle-3-no-component-is-a-singleton)
...
## [Principle 4: Keep the Local State Isolated](#principle-4-keep-the-local-state-isolated)
...
I don’t suggest a dogmatic interpretation of these rules. Of course, in a simpler app you might want to use local state for everything, including those “caches”. I’m only talking about the ideal user experience [from the first principles](https://overreacted.io/the-elements-of-ui-engineering/).
...
**These principles help you write components that are [optimized for change](https://overreacted.io/optimized-for-change/). It’s easy to add, change them, and delete them.**
