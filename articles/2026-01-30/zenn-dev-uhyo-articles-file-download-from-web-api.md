...
source: "https://zenn.dev/uhyo/articles/file-download-from-web-api"
...
今回はAPI[\[1\]](#fn-90db-1)から得られたデータをファイルとしてダウンロードさせたい場合のフロントエンドの実装方法について考察します。
...
1.  この記事では、バックエンドによって実装されHTTPエンドポイントとして公開されているものを指します。これをWeb APIなどと呼ぶ流派もありますが、単にAPIと言っても伝わるので、この記事ではAPIと呼びます。 [↩︎](#fnref-90db-1)