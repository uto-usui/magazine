---
title: "Promise.allSettled と Promise.any"
source: "https://blog.jxck.io/entries/2019-08-20/promise-allsettled-any.html"
...
## [Promise.all()/race()](#promise.allrace)
...
すると、「_全ての処理が完了してほしい_」という要件を満たすためには、リトライする必要がある。
...
従って、「_最初に Resolve する Promise を待ちたい_」という要件を満たす場合は、リトライする必要がある。
...
従って、「_最初に Resolve する Promise を待ちたい_」という要件を満たす場合は、any を使えばそれだけで良い。

## [補足](#補足)

Promise が Resolve した状態を Fulfilled, Reject した状態を Rejected と言う。

Fulfilled か Rejected どちらかになった状態、つまり成功失敗に関わらず「処理が終わった状態」を _Settled_ と言う。
...
-   [https://labs.jxck.io/promise/](https://labs.jxck.io/promise/)