...
source: "https://overreacted.io/a-lean-syntax-primer/"
...
### [Declaring Definitions](#declaring-definitions)

Let’s start by writing a few _definitions_. These can appear at the top level of the file:
...
_([Try it in the online playground.](https://live.lean-lang.org/#codez=CYUwZgBAdghgtiCAuAvBARAQQDYEsDGI6AUKJDAOaKoQAsATMUA))_
...
If you hover on `temperature`, you’ll see that it’s an `Int`. An `Int`, which stands _for integer_, is a built-in type allowing any whole number, negative or positive.
...
One way to use Lean is to run your code. Another way to use Lean is to prove some facts _about_ your code. You can also do both—write code and proofs about it. We’re going to start by learning to run some code, and then we’ll look at writing proofs.
...
Hovering over this `#eval` in your editor will now say `1983`. Another place where it’ll show up is the _InfoView_ on the right side of the online playground:

![1983 shows up in InfoView](https://overreacted.io/a-lean-syntax-primer/1.png)
...
![Screenshot of InfoView in VS Code](https://overreacted.io/a-lean-syntax-primer/2.png)
...
Now let’s _prove_ that `age + birthYear` together add up to `2025`.
...
![Goal: age + birthYear = 2025](https://overreacted.io/a-lean-syntax-primer/7.png)
...
![Goal: 42 + birthYear = 2025](https://overreacted.io/a-lean-syntax-primer/8.png)
...
![Goal: 42 + (2025 - age) = 2025](https://overreacted.io/a-lean-syntax-primer/9.png)
...
![Goal: 42 + (2025 - 42) = 2025](https://overreacted.io/a-lean-syntax-primer/10.png)
...
![No goals](https://overreacted.io/a-lean-syntax-primer/11.png)

And you’re done! You’ve now proven that `age + birthYear = 2025` without actually having _run_ any code. This is being verified _during typechecking._
...
![tactic 'decide' proved that 42 + (2023 - 42) is false](https://overreacted.io/a-lean-syntax-primer/12.png)
...
![simp [age, birthView] solves the same theorem](https://overreacted.io/a-lean-syntax-primer/13.png)

This also proves the goal.

This example is contrived but I wanted to show you how it feels to step through the code step by step and transform it “outside in”. It almost makes you feel like _you’re_ the computer, logically transforming the code towards the goal. It won’t always be so tedious, especially if you have some useful theorems prepared.
...
You could avoid the need to write `IO.` before it by _opening_ the `IO` namespace:
...
However, you would sometimes use parentheses _around_ individual arguments. Suppose you replace `birthYear` with `2025 - age` directly in the function call:
...
![Failed to synthesize HSub (IO Unit) Nat ?m.342](https://overreacted.io/a-lean-syntax-primer/3.png)
...
### [Nesting Expressions](#nesting-expressions)
...
You can turn `birthYear` into a _function_ definition by declaring a `currentYear` argument immediately after writing `def birthYear` followed by a space:
...
![](https://overreacted.io/a-lean-syntax-primer/5.png)
...
### [Many Ways to Declare a Function](#many-ways-to-declare-a-function)
...
![typeclass instance is stuck](https://overreacted.io/a-lean-syntax-primer/14.png)
...
### [Proving For All](#proving-for-all)
...
These proofs typecheck, but there’s a feeling that it’s not any better than writing tests. What we’re hoping to capture is a _universal pattern_, not a bunch of cases.
...
![Automatically inserted implicit arguments: a : Nat, cy : Int](https://overreacted.io/a-lean-syntax-primer/15.png)
...
![Goal: ↑a + birthYear cy a = cy](https://overreacted.io/a-lean-syntax-primer/16.png)

The `⊢` symbol before it tells you that this is the goal, i.e. what you want to prove. Things above it, like `cy : Int` and `a : Nat`, are the things you _already have._

You might be wondering: what _are_ those `cy : Int` and `a : Nat`. What _are_ their values? But the question doesn’t make sense. You’re writing a proof, so in a sense, you are working with _all possible values at the same time._ You only have their types.
...
![@Nat.cast Int instNatCastInt a : Int](https://overreacted.io/a-lean-syntax-primer/17.png)
...
![Goal: ↑a + (cy - ↑a) = cy](https://overreacted.io/a-lean-syntax-primer/18.png)
...
![No goals](https://overreacted.io/a-lean-syntax-primer/19.png)
...
In a way it’s like a test, but it’s a test for _all possible inputs of that function_ that runs during typechecking. I think this is incredibly cool. You can verify that incorrectly changing the formula of `birthYear` will cause the proof to no longer typecheck:

![omega could not prove the goal](https://overreacted.io/a-lean-syntax-primer/20.png)
...
This theorem says “for all `cy` and `a`, `a + birthYear cy a` is equal to `cy`”. Mathematicians have their own way of writing statements like this, using the _universal quantifier_ ∀ (which stands for “for all”): _∀ cy a, a + birthYear(cy, a) = cy_.
...
![intro cy a brings cy : Int and a : Nat into tactic state](https://overreacted.io/a-lean-syntax-primer/24.png)

Again, you don’t have their _concrete_ values because a proof is supposed to work for all possible values. You don’t know anything about them except their types.
...
To learn what a `Prop` is, check out [my previous article](https://overreacted.io/beyond-booleans/).
...
![IO.println.{u_1} {a : Type u_1} [ToString α] (s: α) : IO Unit](https://overreacted.io/a-lean-syntax-primer/25.png)
...
The key insight is that the parts in `{` `}` and `[` `]` are not something you pass to the function directly. That’s why you’re able to just write `IO.println year`. However, these _are_ actual arguments—they’re just filled in by Lean itself.
...
So if you Lean fills them in for you, where _are_ they coming from?

The parts in `{` `}` are called _ordinary implicit parameters_, and they are filled in by type inference based on the parameters that have already been passed in. If Lean can’t unambiguously fill them in, it will complain and would not typecheck. Here, the `α` argument is implicit, but it’s determined by what _you_ passed as an explicit `(s : α)` argument. You’re printing an `Int`, you passed `(s: Int)`, so `α` is `Int`. This is similar to generics, but in Lean generics are just arguments (often implicit).

The parts in `[` `]`, like `[ToString α]`, are called _instance implicit parameters_ and are filled in automatically depending on the code you’ve imported so far. Lean has a mechanism that lets libraries declare implementations of certain intefaces—for example, “how to subtract `Int` and `Int`” or “how to convert `Nat` to `Int`” or “how to turn an `Int` to a `String`”. The `println` function wants a `ToString` implementation that knows how to turn whatever _you_ passed (an `Int`) to a string. Lean has a `ToString Int` implementation in core, so that’s what `println` gets.
...
![Int](https://overreacted.io/a-lean-syntax-primer/26.png)

And the `[ToString]` instance implicit parameter received `instToStringInt`:

![instToStringInt](https://overreacted.io/a-lean-syntax-primer/27.png)
...
### [Command+Click Anything](#commandclick-anything)

There’s a _lot_ more I would’ve liked to cover but I’m running out of steam—and this article is already long. The good news is, there’s a lot you can learn by peeking under the hood on your own because Lean is extremely Command+Click-able.
