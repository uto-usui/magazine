---
title: "How Mux shipped durable video workflows with their @mux/ai SDK"
...
## [Link to heading](#the-problem-with-ai-video-pipelines)The problem with AI video pipelines
...
## [Link to heading](#why-workflow-devkit-was-the-right-fit)Why Workflow DevKit was the right fit
...
## [Link to heading](#how-it-works-in-practice)How it works in practice

Here's what a durable video AI workflow looks like with `@mux/ai`:

```
import { getSummaryAndTags, getModerationScores } from '@mux/ai/workflows';export async function processVideo(assetId: string) {  "use workflow";  const summaryResp = await getSummaryAndTags(assetId);  // ✅ Step succeeds. The summaryResp is persisted.  const moderationResp = await getModerationScores(assetId, {    thresholds: { sexual: 0.7, violence: 0.8 }  });  // ❌ Step fails. Workflow is suspended.  // ✅ Replay happens and picks back up right here  //    without re-doing the getSummaryAndTags work above.  // ✅ Step succeeds. The moderationResp is persisted.  // With Workflow DevKit, you can nest your own "use workflow"  // and "use step" functions inside a larger "use workflow"  const emailResp = await emailUser(assetId);  // ✅ The nested workflow succeeds. Each step  //    inside your emailUser workflow is treated as an  //    isolated step.  return { summaryResp, moderationResp, emailResp };}
```
...
![](https://vercel.com/vc-ap-vercel-marketing/_next/image?url=https%3A%2F%2Fassets.vercel.com%2Fimage%2Fupload%2Fcontentful%2Fimage%2Fe5382hct74si%2F7CMJuh25laP8HDZ2Usk4Fx%2Fa9cb620e0db6788571e462beb1eee4ad%2FScreenshot_2026-01-08_at_11.08.02_PM.png&w=1920&q=75)
...
## [Link to heading](#deploy-anywhere,-scale-on-vercel)Deploy anywhere, scale on Vercel
...
![](https://vercel.com/vc-ap-vercel-marketing/_next/image?url=https%3A%2F%2Fassets.vercel.com%2Fimage%2Fupload%2Fcontentful%2Fimage%2Fe5382hct74si%2F16p5uSZfSWUrFLa9RWepWQ%2F146b9aa115e14132938b59d935f0606c%2F5cc726736324ebc73a9c8ce52ca1aeab7a9ab241-1600x1073.jpeg&w=1920&q=75)
...
## [Link to heading](#what-you-can-build)What you can build
