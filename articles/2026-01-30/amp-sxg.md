---
title: "AMP SXG 対応"
source: "https://blog.jxck.io/entries/2020-12-25/amp-signed-http-exchange.html"
...
したがって、AMP ページを表示すると、その URL バーが Google の URL になり、その仕組自体が色々と物議を呼んだりした。
  ...
  ![SXG に対応してない AMP は URL バーが google のドメインになる](https://blog.jxck.io/entries/2020-12-25/amp-sxg-before.png?210518_024051)

Google は AMP に SXG を組み合わせることで、Google の AMP Cache から提供するが URL バーを Origin のものにできる _AMP SXG_ に対応している。
...
### [amppkg.toml](#amppkg.toml)

amppackager の設定ファイルは toml で用意する。詳細は example を参照。

-   [https://raw.githubusercontent.com/ampproject/amppackager/releases/amppkg.example.toml](https://raw.githubusercontent.com/ampproject/amppackager/releases/amppkg.example.toml)
...
Devtools で見ると以下のように確認できる。
  ...
  ![Devtools での SXG の Debug dump](https://blog.jxck.io/entries/2020-12-25/amp-sxg-devtools.png?210518_024051)
...
通常ページはヘッダーに稲妻アイコンがあり AMP ページへリンクしているが、そのアイコンが無いことからもこれが AMP ページであることが確認できる。
  ...
  ![検索結果からの遷移で AMP SXG が表示され URL バーが blog.jxck.io になっている](https://blog.jxck.io/entries/2020-12-25/amp-sxg-after.png?210518_024051)

また、Search Result を表示した時点で可能性のある SXG が Preload されていることもわかる。

  ![Google 検索結果のページで AMP SXG が Preload されている](https://blog.jxck.io/entries/2020-12-25/sxg-search-preload.png?210518_024051)
...
-   Other
    ...
    -   Serve AMP using signed exchanges
        -   [https://amp.dev/documentation/guides-and-tutorials/optimize-and-measure/signed-exchange/?format=websites](https://amp.dev/documentation/guides-and-tutorials/optimize-and-measure/signed-exchange/?format=websites)
    -   amphtml/amp-cache-transform.md
        -   [https://github.com/ampproject/amphtml/blob/master/spec/amp-cache-transform.md](https://github.com/ampproject/amphtml/blob/master/spec/amp-cache-transform.md)