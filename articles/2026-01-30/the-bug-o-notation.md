...
Big-O is a measure of **how much slower the code will get as you throw more data at it**. For example, if a sorting algorithm has O(_n2_) complexity, sorting ×50 times more items will be roughly 502 = 2,500 times slower. Big O doesn’t give you an exact number, but it helps you understand how an algorithm _scales_.

Some examples: O(_n_), O(_n_ log _n_), O(_n2_), O(_n!_).
...
**In other words, the Bug-O of this approach is 🐞(_n!_)** where _n_ is the number of code segments touching the DOM. Yeah, that’s a _factorial_. Of course, I’m not being very scientific here. Not all transitions are possible in practice. But on the other hand, each of these segments can run more than once. 🐞(_¯\\\_(ツ)\_/¯_) might be more accurate but it’s still pretty bad. We can do better.
