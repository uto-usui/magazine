---
title: "mixed contents 対応を促進する CSP ディレクティブ"
source: "https://blog.jxck.io/entries/2017-01-10/mixed-contents.html"
...
## [mixed contents](#mixed-contents)

HTTPS で配信されたコンテンツが、サブリソースとして HTTP のコンテンツを含む場合、これを _mixed contents_ という。
...
## [mixed contents の挙動](#mixed-contents-の挙動)
...
-   DEMO: [https://labs.jxck.io/mixed/mixed.html](https://labs.jxck.io/mixed/mixed.html)

### [URL バーの変化](#url-バーの変化)

具体的には、多くのブラウザが URL バーの表示を変更し、ユーザに注意を促すようになる。

  ![Mixed Contents があるため TLS が正しくとも URL バーがグレーになっている](https://blog.jxck.io/entries/2017-01-10/insecure-url-bar.png?200901_002432 "url bar with mixed contents")

mixed contents が無ければ本来は以下のようになる。

  ![Mixed Contents が無いため URL バーがグリーンになっている](https://blog.jxck.io/entries/2017-01-10/secure-url-bar.png?200901_002432 "url bar without mixed contents")

証明書の設定が正しくとも、それが EV であろうとも、URL バーが緑にならないことは、信頼が揺らいでいることを意味する。

### [Mixed Active(Script) Contents](#mixed-activescript-contents)

以下のタグ、及びスクリプト等は、_Mixed Active Contents_ と呼ばれる。
...
-   DEMO: [https://labs.jxck.io/mixed/mixed.html#active](https://labs.jxck.io/mixed/mixed.html#active)

### [Mixed Passive(Display) Contents](#mixed-passivedisplay-contents)
...
-   DEMO: [https://labs.jxck.io/mixed/mixed.html#passive](https://labs.jxck.io/mixed/mixed.html#passive)

## [CSP による Mixed Contents 対策](#csp-による-mixed-contents-対策)

Mixed Contents に対して、対策となりえる CSP のディレクティブを解説する。

### [Upgrade-Insecure-Request](#upgrade-insecure-request)
...
-   DEMO: [https://labs.jxck.io/mixed/mixed.html?upgrade-insecure-request](https://labs.jxck.io/mixed/mixed.html?upgrade-insecure-request)

理想はコンテンツ内の全てのリンクを修正したいところだ。

### [Block-All-Mixed-Contents](#block-all-mixed-contents)
...
-   DEMO: [https://labs.jxck.io/mixed/mixed.html?block-all-mixed-content](https://labs.jxck.io/mixed/mixed.html?block-all-mixed-content)

### [mixed contents の発生状況を収集する](#mixed-contents-の発生状況を収集する)
...
## [iframe の中の mixed contents](#iframe-の中の-mixed-contents)
...
広告は入れ子の iframe で作られているものが多い。
  ...
  ![典型的な nested iframe](https://blog.jxck.io/entries/2017-01-10/iframe.png?200901_002432 "ディスプレイ広告の基礎とセキュリティ(P7) by Kenta Suzuki")
...
-   DEMO: [https://labs.jxck.io/mixed/iframe.html](https://labs.jxck.io/mixed/iframe.html)

もし大元のコンテンツに `Upgrade-Insecure-Request` をつければ、iframe 内のサブリソースも全て https に読み替えられるため、サーバが対応していればそれで済む。

-   DEMO: [https://labs.jxck.io/mixed/iframe.html?upgrade-insecure-request](https://labs.jxck.io/mixed/iframe.html?upgrade-insecure-request)

また、`Block-All-Mixed-Contents` をつければ、iframe 内のサブリソースはブロックされるため、mixed contents を回避することはできる。

しかし、_ネストした iframe 内の mixed contents をブロックしても report は発生しない_。

もし iframe の内側の mixed contents まで report ができてしまうと、その内容を把握できる可能性などセキュリティ上問題があるからだ。

-   DEMO: [https://labs.jxck.io/mixed/iframe.html?block-all-mixed-content](https://labs.jxck.io/mixed/iframe.html?block-all-mixed-content)
