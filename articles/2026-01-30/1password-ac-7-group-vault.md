---
title: "1Password AC #7: Group と Vault の運用"
source: "https://blog.jxck.io/entries/2025-11-10/group-vault.html"
...
-   チームメンバー (Team Member)
  ...
  ![default groups](https://blog.jxck.io/entries/2025-11-10/default-groups.png?251107_225907)

この Vault に振られたデフォルトの権限は以下だ。

  ![default permission](https://blog.jxck.io/entries/2025-11-10/default-permission.png?251107_225907)
...
これは、「ドメイン侵害レポート」という機能を有効にすることで作られ、そのレポートを見ることができる権限が付与される Vault だ。
  ...
  ![ドメイン侵害レポート](https://blog.jxck.io/entries/2025-11-10/domain-breach-report.png?251107_225907)

-   ドメイン侵害レポート | 1Password
    -   [https://1password.com/jp/business/domain-breach-report/](https://1password.com/jp/business/domain-breach-report/)
...
### [Vault へのアクセス権](#vault-へのアクセス権)

Vault へのアクセス権の管理は、ダッシュボード上で行う。

  ![Vault へのアクセス権](https://blog.jxck.io/entries/2025-11-10/shared-vault.png?251107_225907)
...
![想定する組織の構成](https://blog.jxck.io/entries/2025-11-10/organization.svg?251107_225907)
...
## [Team Group/Vault](#team-groupvault)

まずは素直に Frontend/Backend の Group を作り、そこにメンバーをアサインする。ここで同時に Vault も作成する。

  ![Group の作成](https://blog.jxck.io/entries/2025-11-10/create-group.png?251107_225907)

作成した Group のメンバーを整理する。デフォルトでは管理者も追加されるが、ダッシュボードから操作できるため、追加しておく必要は特になさそうだ。ここでは Alice と Charlie を追加する。

  ![Frontend Group の設定](https://blog.jxck.io/entries/2025-11-10/frontend-group.png?251107_225907)
...
次に Vault を整理する。Frontend Group は最初から割り当てられている。所有者 Group も追加されているが、これは削除できない。Vault 内は企業の資産とも言えるため、所有者 Group はデフォルトなのだろう。ただし、管理はあくまで管理者 Group で行っているため、こちらには管理者 Group も追加した。
  ...
  ![Frontend Vault の設定](https://blog.jxck.io/entries/2025-11-10/frontend-vault.png?251107_225907)
...
そこで Vault を持たない Manager Group を作り、Alice と Bob を追加する。
  ...
  ![Manager Group](https://blog.jxck.io/entries/2025-11-10/manager-group.png?251107_225907)

ここに、チームメンバーよりも強い権限として、例えば「アカウントの復旧」を付与する。

  ![アカウントの復旧権限を Manager Group に付与する](https://blog.jxck.io/entries/2025-11-10/permission.png?251107_225907)

次に Vault だが、方針は大きく 2 つ考えられる。

まず、作成した Manager Group を Frontend/Backend 両方のグループに追加する方法だ。

  ![Vault に Manager を追加する](https://blog.jxck.io/entries/2025-11-10/manager-to-vault.png?251107_225907)
...
多くの場合はこれで足りるだろうが、もし Alice だけが Frontend Vault の管理者にしたいのであれば、個別に Alice だけを追加することもできる。
  ...
  ![Vault に Alice を追加する](https://blog.jxck.io/entries/2025-11-10/alice-to-vault.png?251107_225907)
