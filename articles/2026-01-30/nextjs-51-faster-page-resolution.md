---
title: "Next.js 5.1: Faster Page Resolution"
source: "https://vercel.com/blog/next5-1"
...
## [Link to heading](#faster-page-resolution)Faster page resolution
...
![Page resolution shown per request. Left is Next.js 5.0, right is Next.js 5.1](https://vercel.com/vc-ap-vercel-marketing/_next/image?url=https%3A%2F%2Fassets.vercel.com%2Fimage%2Fupload%2Fcontentful%2Fimage%2Fe5382hct74si%2FH9qnkaD8bfq1VIaZeEOe0%2Ff69cde4680123a2e3144c9ae3c932252%2Fnext-start-faster-resolution.png&w=1920&q=75)

Page resolution shown per request. Left is Next.js 5.0, right is Next.js 5.1

## [Link to heading](#environment-configuration)Environment configuration
...
```
module.exports = {  serverRuntimeConfig: { // Will only be available on the server side    mySecret: 'secret'  },  publicRuntimeConfig: { // Will be available on both server and client    staticFolder: '/static'  }}
```
...
```
import getConfig from 'next/config'const {serverRuntimeConfig, publicRuntimeConfig} = getConfig()console.log(serverRuntimeConfig.mySecret) // Will only be available on the server sideconsole.log(publicRuntimeConfig.staticFolder) // Will be available on both server and clientexport default () => <div>  <img src='\${publicRuntimeConfig.staticFolder}/logo.png' /></div>
```

The \`getConfig\` method from the \`next/config\` module is used to get configuration values

## [Link to heading](#improved-error-handling)Improved Error Handling
...
## [Link to heading](#phases-/-config-function)Phases / config function
...
## [Link to heading](#improved-production-source-map-generation)Improved production source map generation
...
```
module.exports = {  webpack(config, {dev}) {    if (!dev) {      config.devtool = 'source-map';      for (const plugin of config.plugins) {        if (plugin['constructor']['name'] === 'UglifyJsPlugin') {          plugin.options.sourceMap = true;          break;        }      }    }    return config  }}
```
...
```
const withSourceMaps = require('@zeit/next-source-maps')module.exports = withSourceMaps()
```
...
![The source code is shown in the sources panel](https://vercel.com/vc-ap-vercel-marketing/_next/image?url=https%3A%2F%2Fassets.vercel.com%2Fimage%2Fupload%2Fcontentful%2Fimage%2Fe5382hct74si%2F4trMyTKBgQDFr0Ny94Ia4W%2F6da7ddbc589a431ac81765a373de45be%2Fnextjs-sourcemap.png&w=1920&q=75)

The source code is shown in the sources panel

## [Link to heading](#new-plugins-/-improvements-to-existing-ones)New plugins / improvements to existing ones
