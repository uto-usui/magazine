...
source: "https://ics.media/entry/250627/?utm_source=atom&utm_medium=referral&utm_campaign=feed"
...
`interpolate-size`プロパティと`calc-size()`関数はChrome・Edge 129（2024年9月）以上でサポートされています。2026年1月現在、Safari、Firefoxはサポート対象外なのでご注意ください。

参照：[interpolate-size | Can I use…](https://caniuse.com/mdn-css_properties_interpolate-size)

参照：[calc-size() | Can I use…](https://caniuse.com/mdn-css_types_calc-size)
...
アコーディオンやアクションシートに`interpolate-size`とスプリングアニメーションを組み合わせたUI例は、記事「[CSSのlinear()でUIが軽快になる! スプリングアニメーション活用術](https://ics.media/entry/260402/)」で紹介しています。
