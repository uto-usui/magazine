...
source: "https://ics.media/entry/240418/?utm_source=atom&utm_medium=referral&utm_campaign=feed"
...
```
<input pattern="\d{3}-\d{4}" title="123-4567 の形式で入力してください" />
```

`pattern`にマッチしないときのブラウザ標準メッセージは抽象的になりがちです。`title`属性を併用するとネイティブのバリデーションUIでも形式のヒントを表示できます。テキストで常に表示する場合は後述の`setCustomValidity()`を使います。
...
この欠点を補うための疑似クラスが`:user-valid`、`:user-invalid`疑似クラスです。これらの疑似クラスは**ユーザーの入力後**にバリデーション結果に応じて付与されます。2023年11月時点で主要ブラウザが対応しBaselineの対象になっており、未入力の必須項目はユーザーが操作するか送信を試みるまで`:user-invalid`に一致しません。
...
バリデーションは支援技術でも正しく伝わるように設計しましょう。入力欄には`<label>`を付け、ヒントやエラーメッセージは`aria-describedby`で関連付けます。エラー時は`aria-invalid="true"`を付け、即時に読ませたい場合はメッセージ要素に`role="alert"`を付けます。CSSの`:user-invalid`は支援技術の状態と自動では連動しない点に注意してください。
...
```
<input required minlength="5" maxlength="10" aria-describedby="message" />
...
`aria-describedby`でエラーメッセージ要素と入力欄を関連付け、検証結果に応じて`aria-invalid`を切り替えます。
...
これにより、パターンミスマッチの場合は定義したエラーメッセージが、それ以外のエラーの場合はブラウザから提供されるエラーメッセージが表示されるようになりました。`setCustomValidity()`でメッセージを上書きしたあとは、検証のたびに`setCustomValidity("")`でいったんリセットしましょう。一度設定したメッセージは自動では消えないためです。

ほかにも`reportValidity()`があります。通常の送信では、ブラウザが自動でバリデーションし、エラーがあれば標準のバルーンを表示します。送信処理をJavaScriptで止めたうえでバルーンを出したいとき、または`<form novalidate>`で送信時の自動チェックを無効にしているときは、`reportValidity()`を呼びます。
...
ただし、この実装には注意点があります。HTML標準機能では送信時にバリデーションエラーが表示されていたのが、送信ボタンが押せなくなったことによってバリデーションエラーの内容がユーザーにわからなくなることがあります。ボタンの活性状態を制御する際は、**必ずバリデーションエラーメッセージを表示する**ようにしましょう。 `disabled`属性で無効化すると、ボタンはキーボードでフォーカスできなくなります。 `pointer-events: none`はクリックだけを防ぎ、キーボードではフォーカス・操作できる場合があります。
