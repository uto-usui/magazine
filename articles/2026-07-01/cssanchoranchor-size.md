...
source: "https://ics.media/entry/251215/?utm_source=atom&utm_medium=referral&utm_campaign=feed"
...
CSSアンカーポジショニングの応用として、選択したボタンに、背景が追従するようなモーションを加えています。実装の考え方は前段落『3. 応用 - リストの「今見ている要素」を指し示す』と同じです。

-   [サンプルを別ウインドウで開く](https://ics-creative.github.io/260128_css_anchor_positioning/anchor_pos_example.html)
-   [コードを確認する](https://github.com/ics-creative/260128_css_anchor_positioning/blob/main/anchor_pos_example.html)

### ブラウザ対応状況

`anchor()`関数と`anchor-size()`関数は、2026年1月現在、Chrome・Edge 125（2024年5月）以上、Safari 26.0（2025年9月）、Firefox 147（2026年1月）以上で利用できます。

参照：[“anchor()” | Can I use…](https://caniuse.com/mdn-css_types_anchor)

参照：[“anchor-size()” | Can I use…](https://caniuse.com/mdn-css_types_anchor-size)

ただし、Firefoxでは`anchor()`関数と`anchor-size()`関数の`transition`が効かないことに注意が必要です。
...
アンカーポジショニングをセグメントボタンやモーダルUIのモーションに応用した作例は、記事「[CSSのlinear()でUIが軽快になる! スプリングアニメーション活用術](https://ics.media/entry/260402/)」でも紹介しています。
