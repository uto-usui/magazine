...
source: "https://ics.media/entry/230525/?utm_source=atom&utm_medium=referral&utm_campaign=feed"
...
※本記事はStylelint 17.14.0、Node.js 24を対象に解説しています
...
実行時に`stylelint.config.mjs`ファイルを自動で検出して、Stylelintの設定ファイルとして扱います。設定ファイルは`stylelint.config.js`といった別の拡張子でも利用できます。

-   [Configuring | Stylelint](https://stylelint.io/user-guide/configure/)

今回は、`stylelint.config.mjs`を`package.json`と同じディレクトリに作成して、`no-duplicate-selectors`という「同じセレクターの重複を禁止する」ルールを追加してみます。
...
なお、公式の[`create-stylelint`](https://www.npmjs.com/package/create-stylelint)を使えば、`stylelint-config-standard`（CSS向け）の導入までまとめて行えます。

```
npm create stylelint@latest
```

以下では、設定の中身を理解するために、手動で共有設定を追加する流れを紹介します。
...
また、`extends`を設定したうえで、ルールを拡張したり、プロジェクトに適さないルールを上書きしたい場合は`rules`フィールドを指定することで上書きできます。次の例は、`stylelint-config-standard`で有効な`no-duplicate-selectors`を無効にしています。
...
-   [stylelint-config-standard-scss](https://github.com/stylelint-scss/stylelint-config-standard-scss)
    -   `stylelint-config-standard`と[stylelint-config-recommended-scss](https://github.com/stylelint-scss/stylelint-config-recommended-scss)を拡張した設定ファイルで、CSSに加えてSass（SCSS記法）でもStylelintを利用できます。ICSではSass（SCSS記法）を利用することが多く、厳格なルールで運用するため採用しています。
...
`npm scripts`については詳しく知りたい方は、記事『[Node.jsユーザーなら押さえておきたいnpm-scriptsのタスク実行方法まとめ](https://ics.media/entry/12226/)』を参考にしてください。

Stylelintでは**一部のルールで自動修正**が可能です。コマンドに`--fix`引数を追加することで実行できます。該当のルールで発生したエラーを修正するために`--fix`を追加したコマンドを管理すると便利です。
...
ESLintでも同様に`--fix`があり、Lintコマンドの実行タイミング（エディター、コミット前、CI）をあわせて設計する考え方は共通です。詳しくは、記事『[ESLintを「入れただけ」で終わらせない — 適切なルールの選び方と実行タイミング](https://ics.media/entry/260410/)』を参考にしてください。
...
-   対象外のファイルを`ignoreFiles`で指定するか、`.stylelintignore`ファイルを用意する
