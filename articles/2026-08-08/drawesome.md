...
`placement` puts it on an edge and `inset` and `align` say exactly where. `motion` is how it arrives and leaves, coming in off whichever edge it's on. `draggable` lets people move it themselves. `swatches` replaces the palette with your own colours. `look="studio"` lights the tools as objects instead of shading them flat, and `depth` sets how physical the bar looks.
