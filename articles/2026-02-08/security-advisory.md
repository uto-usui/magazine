...
source: "https://storybook.js.org/blog/security-advisory/"
...
No one has reported an exploited project to us. Once we learned of the vulnerability, we worked diligently to release patched versions and distribute a [GitHub security advisory](https://github.com/storybookjs/storybook/security/advisories/GHSA-8452-54wp-rmv6?ref=storybookblog.ghost.io) (CVE-2025-68429).
...
process.env.MY_VAR // Safe, if MY_VAR is available (see next pattern)
```

Or if they reference any `process.env.*` variable when that variable is not available to Storybook. Variables are available to Storybook when they are defined in a `.env` file or by being explicitly set in [Storybook’s configuration](https://storybook.js.org/docs/configure/environment-variables?ref=storybookblog.ghost.io#using-storybook-configuration). They are also available when prefixed with `STORYBOOK_`, but we are assuming those values are not secret.
...
-   `@amplitude/storybook-addon-amplitude`
    -   This does not actually contain the vulnerable pattern in the source, but its [documentation](https://github.com/amplitude/storybook-addon-amplitude?ref=storybookblog.ghost.io#getting-started) guides people toward referencing environment variables in `manager.js|ts`.
...
If your project is built and published with Chromatic, proactive actions have been taken to mitigate exposure. More details in [Chromatic’s security advisory](https://chromatic.com/blog/storybook-security-advisory?ref=storybookblog.ghost.io).

Thank you to [Matt G](https://www.linkedin.com/in/mattagill/?ref=storybookblog.ghost.io) for the responsible disclosure.