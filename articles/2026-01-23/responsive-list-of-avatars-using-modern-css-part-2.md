...
source: "https://css-tricks.com/responsive-list-of-avatars-using-modern-css-part-2/"
...
Ready for the second part? If you recall, [last time](https://css-tricks.com/responsive-list-of-avatars-using-modern-css-part-1/) we worked on a responsive list of overlapping avatar images featuring a cut-out between them.
...
The following demo is currently limited to Chrome and Edge, but will work in other browsers as the [`sibling-index()`](https://css-tricks.com/almanac/functions/s/sibling-index/) and [`sibling-count()`](https://css-tricks.com/almanac/functions/s/sibling-count/) functions gain broader support. You can track Firefox support in [Ticket #1953973](https://bugzilla.mozilla.org/show_bug.cgi?id=1953973) and WebKit’s position in [Issue #471](https://github.com/WebKit/standards-positions/issues/471).

We will rely on the same HTML structure and CSS base as the example we covered in [Part 1](https://css-tricks.com/responsive-list-of-avatars-using-modern-css-part-1/): a list of images inside a container with `mask`\-ed cutouts. This time, however, the positions will be different.

#### Responsive List of Avatars Using Modern CSS

1.  **[Horizontal Lists](https://css-tricks.com/responsive-list-of-avatars-using-modern-css-part-1/)**
2.  **Circular Lists** _(_You are here!_)_

### Placing Images Around a Circle

There are several techniques [for placing images around a circle](https://css-tip.com/images-circle/). I will start with my favorite one, which is less known but uses a simple code that relies on the CSS `offset` property.
...
We define a path to be a circle with a radius of `180px`. All the images will “follow” that path, but will initially be on top of each other. We need to adjust their distance to change their position along the path (i.e., the circle). That’s where [`offset-distance`](https://css-tricks.com/almanac/properties/o/offset-distance/) comes into play, which we combine with the [`sibling-index()`](https://css-tricks.com/almanac/functions/s/sibling-index/) and [`sibling-count()`](https://css-tricks.com/almanac/functions/s/sibling-count/) functions to create code that works with any number of elements instead of working with exact numbers.
...
This will place the elements evenly around the circle. To this, we add a rotation equal to `0deg` using [`offset-rotate`](https://css-tricks.com/almanac/properties/o/offset-rotate/) to keep the elements straight so they don’t rotate as they follow the circular path. From there, all we have to do is update the circle’s radius with the value we want.

That’s my preferred approach, but there is a second one that uses the [`transform`](https://css-tricks.com/almanac/properties/t/transform/) property to combine two rotations with a translation:
...
The `j` is similar to `i`, but for the _next_ image in the sequence, meaning we increment the index by 1. That gives us the following CSS calculations for each variable:
...
Our list is circular, but the HTML code is not, so even if the first and last images are visually placed next to each other, in the code, they are not. We cannot link both of them using the [adjacent sibling selector](https://css-tricks.com/almanac/selectors/a/adjacent-sibling/) (`+`). We need two more selectors to cover those edge cases:
