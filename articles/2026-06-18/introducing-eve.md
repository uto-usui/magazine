...
## [Link to heading](#an-agent-is-a-directory)An agent is a directory
...
### [Link to heading](#create-an-eve-agent-in-minutes)Create an eve agent in minutes
...
## [Link to heading](#why-we-built-eve)Why we built eve
...
## [Link to heading](#batteries-included)Batteries included

Everything an agent needs in production ships with the framework.

### [Link to heading](#a-durable-session-for-every-conversation)A durable session for every conversation
...
### [Link to heading](#a-sandbox-for-every-agent)A sandbox for every agent
...
### [Link to heading](#human-in-the-loop-approvals)Human-in-the-loop approvals
...
### [Link to heading](#secure-connections-to-tools,-data,-and-services)Secure connections to tools, data, and services
...
### [Link to heading](#the-same-agent-on-every-channel)**The same agent on every channel**
...
### [Link to heading](#tracing-and-evals-built-in)Tracing and evals built in
...
## [Link to heading](#extend-an-agent-one-file-at-a-time)**Extend an agent one file at a time**
...
### [Link to heading](#add-human-in-the-loop-approval)**Add human-in-the-loop approval**
...
### [Link to heading](#let-the-agent-write-its-own-code)**Let the agent write its own code**
...
### [Link to heading](#delegate-work-to-a-subagent)**Delegate work to a subagent**
...
## [Link to heading](#start-and-interact-with-your-agent)**Start and interact with your agent**
...
### [Link to heading](#run-the-agent-locally)**Run the agent locally**
...
### [Link to heading](#test-the-agent-with-evals)**Test the agent with evals**
...
## [Link to heading](#introduce-the-agent-to-your-team)**Introduce the agent to your team**
...
### [Link to heading](#put-the-agent-on-a-schedule)**Put the agent on a schedule**
...
```
import { defineSchedule } from "eve/schedules";import slack from "../channels/slack.js";export default defineSchedule({  cron: "0 9 * * 1",  async run({ receive, waitUntil, appAuth }) {    waitUntil(      receive(slack, {        message: "Summarize last week's revenue and post it to the team channel.",        target: { channelId: "C0123ABC" },        auth: appAuth,      }),    );  },});
```
...
## [Link to heading](#run-the-agent-like-the-rest-of-your-software)**Run the agent like the rest of your software**
...
## [Link to heading](#how-we-run-vercel-on-eve)**How we run Vercel on eve**
...
### [Link to heading](#the-data-analyst)The data analyst
...
### [Link to heading](#the-autonomous-sdr)The autonomous SDR
...
### [Link to heading](#the-sales-cockpit)The sales cockpit
...
### [Link to heading](#the-support-engineer)The support engineer
...
### [Link to heading](#the-content-agent)The content agent
...
### [Link to heading](#routing-agent)Routing agent
