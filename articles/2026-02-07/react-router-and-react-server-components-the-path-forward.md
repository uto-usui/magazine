...
In a recent blog post we shared a [preview of React Router with RSC support](https://remix.run/blog/rsc-preview). On the surface, it might look like this simply means that Server Components are coming to React Router.

However, the implications are greater than you might expect.

With RSC, so much of what's required to implement React Router's Framework Mode is now provided by React itself. Similar to the architectural shift we made when [Remixing React Router](https://remix.run/blog/remixing-react-router), we're introducing a more powerful RSC-powered Data Mode that brings most of Framework Mode's features to our lower level library APIs.
