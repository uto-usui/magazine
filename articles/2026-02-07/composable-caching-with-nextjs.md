...
We’re working on a simple and powerful caching model for Next.js. In a previous post, we talked about [our journey with caching](https://nextjs.org/blog/our-journey-with-caching) and how we’ve arrived at the `'use cache'` directive.
...
Let’s look at an example using the cached function in a server component with a [closure](https://v0.app/chat/5kD47RIecQK?b=b_rCP4CvfbFFW).
...
You can define the life of the cache with different [profiles](https://nextjs.org/docs/app/api-reference/functions/cacheLife). We include a set of default profiles, but you can define your own custom values if desired.

To invalidate a specific cache entry, you can [tag the cache](https://nextjs.org/docs/app/api-reference/functions/cacheTag) and then call `revalidateTag()`. One powerful pattern is that you can tag the cache _after_ you have fetched your data (e.g. from a CMS):
...
[Learn more in the documentation](https://nextjs.org/docs/app/api-reference/directives/use-cache).