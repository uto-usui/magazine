...
But that is changing. “The edge” isn’t just about static assets anymore. It’s increasingly becoming a place for dynamic assets which require compute. Chris Coyier talks about this in his post: “[It doesn’t much matter how CDNy your Jamstack site is if everything important happens from a single-origin server. Edge functions are probably part of the solution.](https://chriscoyier.net/2022/05/04/it-doesnt-much-matter-how-cdny-your-jamstack-site-is-if-everything-important-happens-from-a-single-origin-server-edge-functions-are-probably-part-of-the-solution/)”.

> Static files on “the edge” is cool and all, but ya know what’s even cooler? Actually _doing stuff_ on the edge.

Kent also talked about “the edge” and what’s coming next in [his talk at Reactathon](https://youtu.be/V5hPAl1q7vo?t=2546).

[![Screenshot from the video Kent’s talk, showing Kent and his title slide with the words “Shipping to the Edge”](https://remix.run/blog-images/posts/remix-and-the-edge/kent-talk.jpg)](https://youtu.be/V5hPAl1q7vo?t=2546)
...
![Illustration depiciting two different users connecting to the same server across the world and being served the same resource.](https://remix.run/blog-images/posts/remix-and-the-edge/ssr.png)
...
![Illustration depiciting two different users across the world connecting to CDN nodes close to their geographic location and being served the same resource.](https://remix.run/blog-images/posts/remix-and-the-edge/ssg.png)
...
![Illustration depiciting two different users across the world connecting to CDN nodes close to their geographic location and being served the same resource which is a document with a loading spinner.](https://remix.run/blog-images/posts/remix-and-the-edge/csr-1.png)
...
![Illustration depiciting two different users across the world bypassing CDN nodes close to their geographic location and instead connecting to the same origin server far from their geographic location to be served unique content.](https://remix.run/blog-images/posts/remix-and-the-edge/csr-2.png)
...
[Incremental static regeneration](https://nextjs.org/docs/basic-features/data-fetching/incremental-static-regeneration) (ISR), [distributed persistent rendering](https://www.netlify.com/blog/2021/04/14/distributed-persistent-rendering-a-new-jamstack-approach-for-faster-builds/) (DPR), and any others we might come up with, are meant to solve some of the problems described above on a case-by-case basis.
...
![Illustration of a gauge with a dial. The gauge reads “Static” on the left and “Dynamic” on the right, with labled points on the spectrum between those two extremes for SSG, ISR, CSR, and SSR, each of which has a line through them to show they’re not needed. The Remix logo is shown below the gauge.](https://remix.run/blog-images/posts/remix-and-the-edge/static-to-dynamic.png)
...
The next generation of “The Edge” — technologies like [Deno deploy](https://deno.com/), [Cloudflare Workers](https://workers.cloudflare.com/), and (increasingly) many others — promise to combine static-content CDNs with dynamic-content servers, so you can deliver static assets and dynamic content requiring compute from the same network—think traditional SSR but distributed, as if an origin server and a CDN had a baby.
...
![Illustration depiciting two different users across the world connecting to next generation edge nodes, which are a visual combination of a traditional CDN node and an origin server, resulting in each user retrieiving tailored content from a server in close proximity to their geographic location.](https://remix.run/blog-images/posts/remix-and-the-edge/edge.png)
...
Streaming on the edge! But that’s a blog post for another day. If you can’t wait, go watch [Ryan’s talk at Reactathon](https://youtu.be/Ck-e3hd3pKw?t=9274).