...
source: "https://nextjs.org/blog/june-2023-update"
...
-   **Per-route incremental migration**: Without a major rewrite of your application, you can move a single route of your application over the App Router and start to take advantage of new features at your own pace. See our [incremental adoption guide](https://nextjs.org/docs/app/building-your-application/upgrading/app-router-migration) or [watch a tutorial](https://www.youtube.com/watch?v=YQMSietiFm0).
...
-   **Initial Announcement**: In May of that year, we [released an RFC](https://nextjs.org/blog/layouts-rfc) to outline our plans for making routing and layouts more flexible.
...
-   **Migration Progress**: We started with replacing Babel _(compilation)_ and Terser _(minification)_ with [SWC](https://nextjs.org/docs/architecture/nextjs-compiler). This has helped improve local iteration speeds and production build times.
...
-   **Long-term Investment**: Keeping great Fast Refresh performance regardless of an applications size means making Next.js operate as incremental as possible during local development, by only bundling and compiling code as needed.
    ...
    Turbopack is currently [available in beta](https://nextjs.org/docs/architecture/turbopack) (`next dev --turbo`).
...
We've been working over the past year to re-write the Next.js documentation from scratch. This work is now live on [nextjs.org/docs](https://nextjs.org/docs). We'd like to highlight some [important pieces](https://twitter.com/delba_oliveira/status/1664323492077256704):
...
We'll be releasing new content in the [documentation](https://nextjs.org/docs), on [Twitter](https://twitter.com/nextjs), [YouTube](https://www.youtube.com/c/VercelHQ), and more.
...
-   **Client Components**: With the recent conversation around Server Components, it's important to note the client components are _not_ a de-optimization. The client is a valid part of the React model and is not going away.
    ...
    You can think of client components as the existing Next.js ecosystem today, where your favorite libraries and tools continue to work. For example, a common question we've seen is whether `"use client"` needs to be added to every single file to make it a client component. This is not necessary, but we understand these concepts are new and will take time to learn. You only need to [mark the top level boundary](https://nextjs.org/docs/getting-started/react-essentials#the-use-client-directive) where you code moves between the server to the client. This architecture allows you to [interweave server and client components together](https://github.com/reactwg/server-components/discussions/5).
