---
title: "Next.js 9.5"
source: "https://nextjs.org/blog/next-9-5"
...
To get the best of both worlds, Next.js introduced **Incremental Static Generation**, updating static content after you have already built your site. By using the [`fallback: true` option in `getStaticPaths`](https://nextjs.org/docs/api-reference/data-fetching/get-static-paths#fallback-false), you can **register** **new static pages** **at _runtime_**.
...
Both incremental features (adding pages and lazily updating them), as well as [preview mode](https://nextjs.org/docs/pages/building-your-application/configuring/preview-mode), are now stable and already fully supported by both `next start` and the [Vercel edge platform](https://vercel.com/) out of the box.
...
![After the first visit following our emoji reaction, a new page generation kicks off in the background. Every single request throughout is served from static cache.](https://nextjs.org/_next/image?url=https%3A%2F%2Fh8DxKfmAPhn8O0p3.public.blob.vercel-storage.com%2Fstatic%2Fblog%2Fnext-9-5%2Freactions-demo.gif&w=1920&q=75)
...
For more details, check out [the `getStaticProps` documentation](https://nextjs.org/docs/pages/building-your-application/data-fetching#getstaticprops-static-generation).
...
For more details, check out [the `basePath` documentation](https://nextjs.org/docs/pages/api-reference/next-config-js/basePath).
...
To learn more about `rewrites` feature check out the [rewrites documentation](https://nextjs.org/docs/pages/api-reference/next-config-js/rewrites).
...
To learn more about `redirects` feature check out the [redirects documentation](https://nextjs.org/docs/pages/api-reference/next-config-js/redirects).
...
To learn more about `headers` feature check out the [headers documentation](https://nextjs.org/docs/pages/api-reference/next-config-js/headers).
...
To learn more about the `trailingSlash` feature check out the [trailingSlash documentation](https://nextjs.org/docs/pages/api-reference/next-config-js/trailingSlash)
...
-   **New documentation**: We've [added extensive documentation](https://nextjs.org/docs/architecture/fast-refresh) that explains what Fast Refresh is, how it works, and what to expect! The documentation will also teach you how to better leverage Fast Refresh by explaining how its error recovery works.
-   **User-code troubleshooting guide**: The new documentation also includes [common troubleshooting steps and tips](https://nextjs.org/docs/architecture/fast-refresh#tips) on how to get the most out of Fast Refresh in development.
...
Like catch-all routes, `slug` will be provided in the [router query object](https://nextjs.org/docs/pages/api-reference/functions/use-router#router-object) as an array of path parts. So, for the path `/blog/foo/bar`, the query object will be `{ slug: ['foo', 'bar'] }`. For the path `/blog`, the query object will omit the slug key: `{ }`.

You can [learn more about optional catch all routes in our documentation](https://nextjs.org/docs/pages/building-your-application/routing/dynamic-routes#optional-catch-all-routes).
...
**Important:** if your project has [custom webpack configuration](https://nextjs.org/docs/pages/api-reference/next-config-js/webpack), some changes might be needed to transition to webpack 5. We recommend keeping an eye out for our migration instructions or minimize your usage of webpack extensions altogether for seamless future upgrades.
