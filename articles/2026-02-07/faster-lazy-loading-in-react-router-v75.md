...
source: "https://remix.run/blog/faster-lazy-loading"
...
In [React Router v6.4](https://reactrouter.com/changelog#v640), we introduced support for [lazy loading of routes](https://remix.run/blog/lazy-loading-routes) via an async `route.lazy()` function. Most commonly this was used to dynamically import a route module, for example:
...
![Waterfall diagram showing `Click /projects/123` with a row for the matching routes of `projects` and `:projectId`, with each matching route’s `route.lazy()` function being called in parallel followed by calling its loader, with each route being handled in parallel](https://remix.run/blog-images/posts/faster-lazy-loading/lazy-with-loaders.png)
...
![Waterfall diagram showing `Click /projects/123` with a row for the matching routes of `projects` and `:projectId`, with each matching route’s `route.lazy()` function being called in parallel, followed by the first route’s middleware being called once all `route.lazy()` functions have resolved, followed by all routes loaders being called in parallel](https://remix.run/blog-images/posts/faster-lazy-loading/lazy-with-loaders-and-middleware.png)
...
![Waterfall diagram showing `Click /projects/123` with a row for the matching routes of `projects` and `:projectId`, with each matching route’s `route.lazy()` function being called in parallel, followed by all route loaders being called in parallel once all `route.lazy()` functions have resolved](https://remix.run/blog-images/posts/faster-lazy-loading/lazy-with-loaders-without-middleware.png)
...
Both of these optimizations allow you to get similar runtime performance to Framework Mode’s [Split Route Modules](https://remix.run/blog/split-route-modules) feature, but since you’re in Data Mode, you now have more control over the file structure.
