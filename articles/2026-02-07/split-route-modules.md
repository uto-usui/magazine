...
![Waterfall diagram showing `Click /route` triggering `Get route module` (split into `clientLoader` and `Component` segments) followed by `Run clientLoader` before `Render content` occurs](https://remix.run/blog-images/posts/split-route-modules/get-route-module.png)
...
![Waterfall diagram showing `Click /route` triggering a parallel `Get clientLoader` and `Get Component` followed by `Run clientLoader` before `Render content` occurs, now earlier than before](https://remix.run/blog-images/posts/split-route-modules/get-route-module-split.png)
...
![Waterfall diagram showing `Click /route` triggering `Get route module` (split into `clientLoader`, `clientAction`, `Component` and `HydrateFallback` segments) followed by `Run clientLoader` before `Render content` occurs](https://remix.run/blog-images/posts/split-route-modules/get-big-route-module.png)

This would instead be optimized to the following:

![Waterfall diagram showing `Click /route` triggering a parallel `Get clientLoader`, `Get clientAction` and `Get Component` (with `HydrateFallback` being skipped) followed by `Run clientLoader` before `Render content` occurs, now earlier than before](https://remix.run/blog-images/posts/split-route-modules/get-big-route-module-split.png)
