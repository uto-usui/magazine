...
Remix is designed to make your application performant by default. Our latest feature, [Fog of War](https://remix.run/docs/en/main/guides/lazy-route-discovery)[1](#user-content-fn-1) (a.k.a. "Lazy Route Discovery")[2](#user-content-fn-2), helps your application stay performant no matter how large it grows.
...
In order to avoid "render then fetch" waterfalls, Remix [decouples rendering from fetching](https://www.youtube.com/watch?v=95B8mnhzoCM) (see also: [Remixing React Router](https://remix.run/blog/remixing-react-router)). In order to do this, Remix needs to know your route tree up front so it can kick off the data fetches and download the route modules in parallel whenever a link is clicked. This results in the inverted and more performant approach of "fetch then render" (or "fetch while render" if you're [streaming data](https://remix.run/docs/en/main/guides/streaming)).
...
![Render then Fetch network diagram](https://remix.run/blog-images/posts/fog-of-war/render-then-fetch.png)

With "fetch then render", the module fetch and data fetch can be parallelized:

![Fetch then Render network diagram](https://remix.run/blog-images/posts/fog-of-war/fetch-then-render.png)

You can take this one step further in Remix via [`<Link prefetch>`](https://remix.run/docs/en/main/components/link#prefetch), which allows you to prefetch the route data and components before a user even clicks a link. That way, when the link is clicked the navigation can be instantaneous:

![Prefetching network diagram](https://remix.run/blog-images/posts/fog-of-war/prefetch.png)
...
To do this, Remix ships a **Route Manifest** to the client that contains all of your route definitions plus a small amount of metadata. This manifest allows Remix to build a client-side route tree without including the underlying route implementations. Those implementations are loaded via [route.lazy](https://reactrouter.com/en/main/route/lazy) when a route is navigated to.

Here's a sample route manifest entry for the root route on [https://remix.run](https://remix.run/):
...
This approach works for most applications - since the manifest is pretty lightweight and compresses well since it's a repetitive key/value JSON structure. For example, the manifest for [https://remix.run/](https://remix.run/) contains 50 routes, weighs 19.6Kb uncompressed, but only sends 2.6Kb over the wire after compression.
...
As we began rolling out Remix to [https://www.shopify.com](https://www.shopify.com/), we realized _just how big_ that site is. When you take into account all of the routes and their internationalized URLs (i.e., `/pricing`, `/en/pricing`, `/es/precios` and many more)—the app had over 1300 routes! And because Remix doesn't have a good solution for URL aliasing (yet!), many of the route entries were duplicates pointing to the same route module, and thus duplicating the module information (its path, its other module `imports`, etc.). This resulted in a manifest that was ~85Kb compressed, and ~10Mb uncompressed. On slower devices, this could have a noticeable impact on page load times for the device to decompress, parse, compile, and execute the JS module.

## Fog of War

At Remix, we're big fans of the Retro vibes of our younger years: from old school web development using HTML `<form>` elements and HTTP `POST` requests, to '90s music, to retro video games with [ever expanding maps](https://en.wikipedia.org/wiki/Fog_of_war). These expanding game maps provided the inspiration for (at the very least) the name of our solution to this problem of ever-growing route manifests.
...
![Remix v0 network diagram](https://remix.run/blog-images/posts/fog-of-war/v0.png)
...
So for Remix 1.0 the full manifest was shipped to eliminate waterfalls and allow link prefetching. The "partial manifest" optimization was left for another day - and that day finally came in Remix [v2.10](https://github.com/remix-run/remix/blob/main/CHANGELOG.md#v2100) with the release of the `future.unstable_fogOfWar` flag (renamed to `future.unstable_lazyRouteDiscovery` in [v2.11](https://github.com/remix-run/remix/blob/main/CHANGELOG.md#v2110))[2](#user-content-fn-2).
...
![Fog of War network diagram with eager discovery](https://remix.run/blog-images/posts/fog-of-war/fow-eager.png)
...
![Fog of War network diagram with eager discovery](https://remix.run/blog-images/posts/fog-of-war/fow-eager-prefetch.png)
...
![Route tree showing the entire manifest without Fog of War enabled](https://remix.run/blog-images/posts/fog-of-war/tree-1.png)

Now, if we enable Fog of War, we'll only ship the active routes in the manifest on initial load:

![Route tree showing the initial manifest with Fog of War enabled](https://remix.run/blog-images/posts/fog-of-war/tree-2.png)
...
![Route tree showing destination links rendered on the current page](https://remix.run/blog-images/posts/fog-of-war/tree-3.png)

Remix will discover those routes via a `fetch` call to the Remix server and patch them into the manifest:

![Route tree with expanded manifest including destination links](https://remix.run/blog-images/posts/fog-of-war/tree-4.png)
...
As mentioned earlier, we've been dog-fooding this on [https://shopify.com](https://shopify.com/) and we're loving the results. Prior to Fog of War, their route manifest contained **1300 routes** and weighed over **10MB uncompressed**. Once Fog of War was enabled, their initial homepage manifest dropped to just **3 routes** and **1.9Kb uncompressed**.

We've also deployed this to [https://remix.run](https://remix.run/), dropping the uncompressed size of the initial manifest from **~20Kb** to **~4Kb**.

## React Router Implementation

As with most routing features in Remix, it's all React Router under the hood. Fog of War is made possible by a new [`unstable_patchRoutesOnMiss`](https://reactrouter.com/en/main/routers/create-browser-router#optsunstable_patchroutesonmiss) API. This API allows you to provide an implementation to add new routes to the route tree anytime React Router is unable to match a path in the current route tree. You can implement any async logic you want in this method to discover the appropriate routes and patch them into the current tree at any location you need.
...
We'd also like to give a huge shout-out to [Shane Walker](https://twitter.com/swalker326) for working with us during the initial release to put together a [great example](https://github.com/swalker326/react-router-fog-of-war-example/) of using this new API in a federated `rsbuild` React Router application. Make sure to give it a look if you're interested in using Module Federation in your React Router app!

## Footnotes

1.  The Fog of War feature was released behind an `unstable` flag in Remix [v2.10](https://github.com/remix-run/remix/blob/main/CHANGELOG.md#v2100) for early beta testing—we hope to stabilize it in an upcoming release [↩](#user-content-fnref-1)
    
2.  This was released behind the `future.unstable_fogOFWar` flag in v2.10, but the flag was renamed to `future.unstable_lazyRouteDiscovery` in v2.11. We found that without the context behind "Fog of War" we discuss in this post, the flag naming could be a bit confusing 🙂. [↩](#user-content-fnref-2) [↩2](#user-content-fnref-2-2)