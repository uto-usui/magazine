...
source: "https://remix.run/blog/remix-data-flow"
publishedDate: "2022-06-22"
category: "frontend"
feedName: "Remix Blog"
---

When React first appeared on the scene, one of its most compelling features was its “one-way data flow”. This is still outlined in the React docs under the page [“Thinking in React”](https://reactjs.org/docs/thinking-in-react.html)
...
![Illustration of the idea of one-way data flow depicting lines drawing a circular flow from view to action to state.](https://remix.run/blog-images/posts/remix-data-flow/view-action-state.png)
...
![Illustration of the flow “View -> Action -> State” framed in a browser on the left. On the right is an illustration of a server with a database. Two arrows connect these two illustrations denoting network transfer.](https://remix.run/blog-images/posts/remix-data-flow/view-action-state-server-client.png)
...
![Illustration of the flow “View -> Action -> State” framed in a browser on the left. On the right is an illustration of a server with a database. Two arrows connect these two illustrations denoting network transfer. The visual emphasis is on the network part of the graphic with “?”s surrounding it.](https://remix.run/blog-images/posts/remix-data-flow/view-action-state-network.png)
...
![Illustration of the flow “View -> Action -> State” crossing between the browser and the server.](https://remix.run/blog-images/posts/remix-data-flow/view-action-state-server-client-network.png)
...
![Illustration depicting a one-way flow of “remote data” between the client/server facilitated by remix. A one-way flow of “local data” is on the client exclusively facilitated by React and/or localStorage.](https://remix.run/blog-images/posts/remix-data-flow/view-action-state-local-vs-remote.png)
...
![Screenshot of code example in Remix illustrating the one-way, cyclical flow of data through an app. There’s a loader function whose code flows into the Route component whose code, via a <Form> flows into the action function whose code flows back into a loader again.](https://remix.run/blog-images/posts/remix-data-flow/loader-action-component-code.png)

Hopefully this helps illustrate how Remix drastically helps reduce the amount of complexity required to build better websites. As Kent says in [his talk at RenderATL](https://youtu.be/zED9ePuht4g?t=24852), because Remix works before JavaScript that’s a win for your users because they get an experience supported by progressive enhancement. But it's also a win for you as a developer because you don’t need to build all the complexity traditionally coupled with state management solutions.
