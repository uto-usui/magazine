...
source: "https://nextjs.org/blog/next-5"
...
Like CSS loaders and pre-processors, TypeScript has been one of the most requested features. To make incorporating it into projects just as easily as any other loader, we now have a [`next-typescript` plugin](https://nextjs.org/) you can include in your `next.config.js` file:
...
All it takes to _"glue them together"_ into a uniform experience for the end-user is [some simple configuration](https://nextjs.org/docs/pages/building-your-application/configuring/absolute-imports-and-module-aliases) of the backend routing layer or load balancers that expose them to the world. We are very happy to now bring the ability to **compose multiple applications built with Next.js**, connected together using normal `<Link>` components. We call this feature **Zones**.
...
![Both of our pages have a seamless experience, but they belong to separate apps](https://nextjs.org/_next/image?url=https%3A%2F%2Fh8DxKfmAPhn8O0p3.public.blob.vercel-storage.com%2Fstatic%2Fblog%2Fnext-5%2Fzones.png&w=3840&q=75)
...
![Every time a change happens inside a PR our bot automatically deploys it](https://nextjs.org/_next/image?url=https%3A%2F%2Fh8DxKfmAPhn8O0p3.public.blob.vercel-storage.com%2Fstatic%2Fblog%2Fnext-5%2Fgithub.png&w=1920&q=75)

Every time a change happens inside a PR our bot automatically deploys it

What we ended up is _two zones_, that are brought together into the parent domain `https://vercel.com` using our [path alias feature](https://nextjs.org/docs/pages/building-your-application/configuring/absolute-imports-and-module-aliases). It looks something like this:
...
With Next.js 5.0 we had an opportunity to take a look at `next build` again, the command you run prior to deploying to production or exporting your Next.js application [as a static site](https://nextjs.org/docs/pages/building-your-application/deploying/static-exports).
...
![Our main application production build now takes 38 fewer seconds to complete](https://nextjs.org/_next/image?url=https%3A%2F%2Fh8DxKfmAPhn8O0p3.public.blob.vercel-storage.com%2Fstatic%2Fblog%2Fnext-5%2Fbuild.png&w=3840&q=75)
...
Next.js build a top-level `<Document>` component that gets server-rendered with each page. [Overloading this component](https://nextjs.org/) puts you in complete control over your markup, enabling many [advanced use cases](https://github.com/vercel/next.js/tree/canary/examples).
...
![Errors now show the correct line, file and function name](https://nextjs.org/_next/image?url=https%3A%2F%2Fh8DxKfmAPhn8O0p3.public.blob.vercel-storage.com%2Fstatic%2Fblog%2Fnext-5%2Ferrors.png&w=3840&q=75)
