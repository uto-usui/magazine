...
source: "https://remix.run/blog/future-flags"
...
Every[1](#user-content-fn-1) framework (or library) out there will _at some point_ have to introduce a breaking change. Something that will cause your code _as it's written today_ to break on the new version. This could result in a build-time (or even worse, a run-time) error. But these changes are good! It's how our frameworks evolve, get faster, adopt new platform features, implement community-driven feature requests, and so on.

Out of this inherent need for breaking changes came the [Semantic Versioning](https://semver.org/) (SemVer) specification which defines that breaking changes dictate a new _major_ release version. This is great because it lets application developers know when they should expect their code to require changes on an upgrade, versus when they should expect the upgrade to "just work". Remember though, you should always be reading the release notes and not just blindly upgrading 😉.

Conveniently enough, the same day I started writing this article, `@devagrawal09` [tweeted](https://twitter.com/devagrawal09/status/1631153991215202304) the following which generated a relevant thread of the current state of frameworks and their handling of "major rewrites."

![Tweet from @devagrawal09 asking 'which javascript framework has lived more than 5 years without causing major rewrites?'](https://remix.run/blog-images/posts/future-flags/tweet.png)

Check back with Remix in 2026!
...
-   Writing a detailed **migration guide** [1](https://reactrouter.com/upgrading/v5#introduction) [2](https://angular.io/guide/upgrade) [3](https://expressjs.com/en/guide/migrating-4.html)
-   Releasing a **preparation version** prior to the major release to better prep your code to adopt the breaking changes [1](https://www.infoworld.com/article/3031266/angular-150-paves-the-way-for-angular-2.html)
-   Releasing a **compatibility package** that allows you to run both versions together [1](https://reactrouter.com/upgrading/v5#backwards-compatibility-package) [2](https://v3-migration.vuejs.org/migration-build.html) [3](https://angular.io/guide/upgrade#upgrading-with-ngupgrade)
...
![Diagram of a long lived feature branch for implementing the changes from a migration guide](https://remix.run/blog-images/posts/future-flags/migration-guide.png)

Long-lived feature branch for a migration guide
...
![Diagram of 2 shorter-lived feature branches for implementing the changes from a preparation version](https://remix.run/blog-images/posts/future-flags/prep-version.png)

2 medium-lived feature branches for a preparation version
...
![Diagram of many short-lived branches to implement features via a migration build](https://remix.run/blog-images/posts/future-flags/back-compat.png)

Migration builds allow iterative feature adoption after the release
...
When we first started talking about how to handle breaking changes for Remix, I couldn't help but think back to the [Stability without Stagnation](https://www.youtube.com/watch?v=R6x7ZGUL3Sk) talk I watched Yehuda Katz give at [Philly ETE 2016](https://2016.phillyemergingtech.com/schedule/). I wasn't an Ember developer but that talk left a huge impression on me[2](#user-content-fn-2) about how frameworks could ease the pain of new feature adoption on their users through the use of feature flags. Ryan Florence, however, _was_ an active Ember developer so when I mentioned this talk he immediately knew the "Stability without Stagnation" phrase.

Later on in my career working on a Vue SSR application, we were preparing for a Vue 2 -> 3 upgrade and I was very excited to see the [Feature Flags](https://v3-migration.vuejs.org/migration-build.html#feature-reference) they were introducing in their build (although I switched jobs prior to performing that upgrade so I don't know how smooth it went).
...
![Diagram of the lack of a feature branches for adopting v2 features via future flags](https://remix.run/blog-images/posts/future-flags/future-flags.png)

Future flags eliminate the need for adoption after the release
...
![Diagram of the gradual adoption of v2 feature via future flags through the v1 lifetime](https://remix.run/blog-images/posts/future-flags/future-flags-v1.png)

Features can be adopted gradually throughout the v1 lifetime
...
[![Flowchart of the decision process for how to introduce a new feature](https://remix.run/blog-images/posts/future-flags/feature-flowchart.png)](https://remix.run/blog-images/posts/future-flags/feature-flowchart.png)

Flowchart for introducing new features (click to open in a new tab)
