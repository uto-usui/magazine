...
source: "https://css-irl.info/debugging-css-grid-part-3-auto-flow-order-and-item-placement/"
...
![A grid with three items](https://css-irl.info/debugging-css-grid-3-01.png)

_Fig 01_Items 2 and 3 overlap, so only one can have its area defined by \`grid-template-areas\`.
...
![Auto placement](https://css-irl.info/debugging-css-grid-3-02.png)

_Fig 02_No items are explicity placed, therefore they are all auto placed.
...
![Three grid items with a mixture of explicit and auto placement](https://css-irl.info/debugging-css-grid-3-03.png)

_Fig 03_The blue item is explicitly placed on the row and column axis (using \`grid-template-columns\` and grid-template-columns\`).
...
![Three grid items with a mixture of explicit and auto placement](https://css-irl.info/debugging-css-grid-3-04.png)

_Fig 04_The blue item is explicitly placed only on the column axis (using grid-template-columns\`).

But if we place the blue item only on the column axis, the items _are_ placed after the first one:

![Three grid items with a mixture of explicit and auto placement](https://css-irl.info/debugging-css-grid-3-05.png)

_Fig 05_The blue item is placed on the column axis with \`grid-column: 2 / 4\`.
...
![Grid items with a mixture of explicit and auto placement](https://css-irl.info/debugging-css-grid-3-06.png)
...
![Grid items with a mixture of explicit and auto placement](https://css-irl.info/debugging-css-grid-3-07a.png)

_Fig 07_ Item 2 has a column span of 3, so will wrap onto the next line.
...
![Auto placed items filling up the explicit grid](https://css-irl.info/debugging-css-grid-3-08a.png)

_Fig 08_Item 2 has a row span of 3
...
Using the `dense` keyword in addition to `row` or `column` (`row` is the default), we can specify that auto placed items should pack themselves into any available grid cells (including ones preceeding any explicitly placed items). This will ensure there are no gaps, even if an item is placed on only one axis. In the above demo you can see how each of the values for `grid-auto-flow` alter the behaviour of the grid.
