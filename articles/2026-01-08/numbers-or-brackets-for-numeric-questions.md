...
source: "https://lea.verou.me/blog/2023/numbers-vs-brackets/"
...
As you may know, this summer I am leading the design of the inaugural [State of HTML](https://lea.verou.me/blog/2023/design-state-of-html/) survey. Naturally, I am also exploring ways to improve both survey UX, as well as all questions.

_Shaine Madala_, a data scientist working on the survey design team [proposed](https://github.com/Devographics/surveys/discussions/166#discussioncomment-6625819) using numerical inputs instead of brackets for the income question. While [I was initially against it](https://github.com/Devographics/surveys/discussions/166#discussioncomment-6626156), I decided to explore this a bit further, which changed my opinion.

![Multiple choice question showing income brackets starting from $0-$10 up to "More than $200k"](https://lea.verou.me/blog/2023/numbers-vs-brackets/images/income-brackets.png)

The current income question, which uses 6 brackets plus a "Not Applicable" option.

There are actually four demographics questions in _State of X_ surveys where the answer is essentially a number, yet we ask respondents to select a bracket: age, years of experience, company size, and income.
...
## [Which one is faster?](#which-one-is-faster%3F)

We can actually calculate this! Average reading speed for non-fiction is around 240 wpm (= 250ms/word) [\[1\]](#fn1)[https://www.sciencedirect.com/science/article/abs/pii/S0749596X19300786](https://www.sciencedirect.com/science/article/abs/pii/S0749596X19300786) Therefore, we can approximate reading time for each question by multiplying number of brackets × average words per bracket (wpb) × 250ms.
...
Average typing speed is 200 cpm [\[2\]](#fn2)[https://www.typingpal.com/en/blog/good-typing-speed](https://www.typingpal.com/en/blog/good-typing-speed) (≈ 300ms/character). This means we can approximate typing time for each question by multiplying the number of digits on average × 300ms.
...
Of course, this is a simplification. There are models in [HCI](https://en.wikipedia.org/wiki/Human%E2%80%93computer_interaction), such as [KLM](https://en.wikipedia.org/wiki/Keystroke-level_model) that can more accurately estimate the time it takes for certain UI flows. We even taught some of these to MIT students in [6.813](http://web.mit.edu/6.813/www/sp18/classes/10-more-efficiency/#keystroke_level_model), as well as [its successor](https://designftw.mit.edu/).
...
Note that this analysis is based on a desktop interface, primarily because it’s easier (most of these models were developed before mobile was widespread, e.g. KLM was invented in 1978!) Mobile would require a separate calculation taking into account the specifics of mobile interaction (e.g. the time it takes for the keyboard to pop up), though the same logic applies. (thanks Tim for [this exellent question](https://twitter.com/gumnos/status/1687199431819763712)!)

## [What about sliders?](#what-about-sliders%3F)
...
When the number is known in advance, sliders are usually a poor choice, except when we have very few numbers to choose among (e.g. a 1-5 rating) and the slider UI makes it very clear where to click to select each of them, or we don’t much care about the number we select (e.g. search flights by departure time).[\[3\]](#fn3)[Slider Design: Rules of Thumb, NNGroup, 2015](https://www.nngroup.com/articles/gui-slider-controls/) None of our demographics questions falls in this category (unless bracketed, in which case why not use regular brackets?).
...
-   Dragging a slider on desktop is generally slower than typing the number outright.[\[4\]](#fn4)KLM is a poor model for dragging tasks for two reasons: First, it regards dragging as simply a combination of three actions: _button press, mouse move, button release_. But we all know from experience that dragging is much harder than simply pointing, as managing two tasks simultaneously (holding down the mouse button and moving the pointer) is almost always harder than doing them sequentially. Second, it assumes that all pointing tasks have a fixed cost (1.1s), which may be acceptable for actual pointing tasks, but the inaccuracy is magnified for dragging tasks. A lot of HCI literature (and even [NNGroup](https://www.nngroup.com/articles/gui-slider-controls/#:~:text=subjected%20to%20the-,steering%20law,-%2C%20which%20describes%20the)) refers to the [Steering Law](https://en.wikipedia.org/wiki/Steering_law) to estimate the time it takes to use a slider, however modern sliders (and scrollbars) do not require steering, as they are not constrained to a single axis: once dragging is initiated, moving the pointer in any direction adjusts the slider, until the mouse button is released. [Fitts Law](https://en.wikipedia.org/wiki/Fitts%27s_law) actually appears to be a better model here, and indeed there are [many papers](https://scholar.google.com/scholar?hl=en&as_sdt=0%2C22&q=fitts+law+dragging) extending it to dragging. However, evaluating this research is out of scope for this post.

## [`<input type=number>` all the things?](#%3Cinput-type%3Dnumber%3E-all-the-things%3F)
...
However, this assumes that the privacy issues are about granularity, or about the number being too low (rounding to 10s could help with both). However, [David Karger](https://people.csail.mit.edu/karger/) made an excellent point in [the comments](https://lea.verou.me/blog/2023/numbers-vs-brackets/#comment-6246846722), that people at the higher income brackets may also be reluctant to share their income:
...
> **What is your approximate yearly income (before taxes)?** $ ,000
...
However, after careful consideration and input, both [qualitative](https://github.com/Devographics/surveys/discussions/177#discussioncomment-6634104) and [quantitative](https://twitter.com/SachaGreif/status/1688447006459805696), it appears that privacy is a much bigger factor than I had previously realized. Even though I was aware that people see income level as sensitive data (more so in certain cultures than others), I had not fully realized the extent of this. In the end, I think **the additional privacy afforded by brackets far outweighs any argument for efficiency or data analysis convenience**.
...
1.  [https://www.sciencedirect.com/science/article/abs/pii/S0749596X19300786](https://www.sciencedirect.com/science/article/abs/pii/S0749596X19300786) [↩︎](#fnref1)
    
2.  [https://www.typingpal.com/en/blog/good-typing-speed](https://www.typingpal.com/en/blog/good-typing-speed) [↩︎](#fnref2)
    
3.  [Slider Design: Rules of Thumb, NNGroup, 2015](https://www.nngroup.com/articles/gui-slider-controls/) [↩︎](#fnref3)
    
4.  KLM is a poor model for dragging tasks for two reasons: First, it regards dragging as simply a combination of three actions: _button press, mouse move, button release_. But we all know from experience that dragging is much harder than simply pointing, as managing two tasks simultaneously (holding down the mouse button and moving the pointer) is almost always harder than doing them sequentially. Second, it assumes that all pointing tasks have a fixed cost (1.1s), which may be acceptable for actual pointing tasks, but the inaccuracy is magnified for dragging tasks. A lot of HCI literature (and even [NNGroup](https://www.nngroup.com/articles/gui-slider-controls/#:~:text=subjected%20to%20the-,steering%20law,-%2C%20which%20describes%20the)) refers to the [Steering Law](https://en.wikipedia.org/wiki/Steering_law) to estimate the time it takes to use a slider, however modern sliders (and scrollbars) do not require steering, as they are not constrained to a single axis: once dragging is initiated, moving the pointer in any direction adjusts the slider, until the mouse button is released. [Fitts Law](https://en.wikipedia.org/wiki/Fitts%27s_law) actually appears to be a better model here, and indeed there are [many papers](https://scholar.google.com/scholar?hl=en&as_sdt=0%2C22&q=fitts+law+dragging) extending it to dragging. However, evaluating this research is out of scope for this post. [↩︎](#fnref4)
...
[Like this post on Bluesky to see your face on this page](https://bsky.app/profile/lea.verou.me/post/3k43dlymwnb2f)
