...
source: "https://csswizardry.com/2015/03/more-transparent-ui-code-with-namespaces/"
...
1.  [The Namespaces](#the-namespaces)
2.  [Object Namespaces: `o-`](#object-namespaces-o-)
3.  [Component Namespaces: `c-`](#component-namespaces-c-)
4.  [Utility Namespaces: `u-`](#utility-namespaces-u-)
5.  [Theme Namespaces: `t-`](#theme-namespaces-t-)
6.  [Scope Namespaces: `s-`](#scope-namespaces-s-)
7.  [Stateful Namespaces: `is-`/`has-`](#stateful-namespaces-is-has-)
8.  [Hack Namespaces: `_`](#hack-namespaces-_)
9.  [JavaScript Namespaces: `js-`](#javascript-namespaces-js-)
10.  [QA Namespaces: `qa-`](#qa-namespaces-qa-)
11.  [Handy Side Effects](#handy-side-effects)
12.  [Detecting Namespaces](#detecting-namespaces)
     1.  [Finding (In)valid Classes](#finding-invalid-classes)
13.  [Highlight Types of Namespace](#highlight-types-of-namespace)
     1.  [Finding Types in Our CSS](#finding-types-in-our-css)
14.  [Too Much to Type?](#too-much-to-type)
15.  [Learning the Namespaces](#learning-the-namespaces)
16.  [An Example](#an-example)
...
This is a naming _convention_. One thing I’ve been researching and implementing a lot with my clients lately is the idea of taking naming conventions a step further by adding _namespaces_.
...
-   `s-`: Signify that a class creates a new styling context or _Scope_. Similar to a Theme, but not necessarily cosmetic, these should be used sparingly—they can be open to abuse and lead to poor CSS if not used wisely.
...
OOCSS is fantastic in that it teaches us to abstract out the repetitive, shared, and purely structural aspects of a UI into reusable _objects_. This means that things like layout, wrappers and containers, the [Media Object](http://www.stubbornella.org/content/2010/06/25/the-media-object-saves-hundreds-of-lines-of-code/), etc. can all exist as non-cosmetic styles that handle the skeletal aspect of a lot of UI components, without ever actually looking like designed ‘things’.
...
Now I can tell _purely_ from this HTML that any changes I make to the `.o-media` class may be felt throughout the entire site, but any changes I make to the `.c-testimonial` ruleset will only modify testimonials, and nothing else.
...
Please see this post’s sister article [Immutable CSS](https://csswizardry.com/2015/03/immutable-css/) for more detail on these kinds of rule.
...
For a really terse but effective example of Scoping styles, see [David Bushell](https://twitter.com/dbushell)’s [Scoping Typography CSS](http://dbushell.com/2012/04/18/scoping-typography-css/).

You **might** want to style this free-form text differently from the rest of the surrounding UI, so you _might_ employ a scoping context. For example:
...
Everything inside the `.s-cms-content` is inaccessible: we can’t get at the DOM to add any classes to the nodes inside of it, so we _might_ begin styling via a Scope. That _might_ look something like this:
...
Stateful namespaces are lovely. They come from [SMACSS](https://smacss.com/book/type-state), and they tell us about short-lived or temporary states of the UI that need styling accordingly.
...
These types of class are pretty easy to spot in our codebase, so any hacks will become very apparent, which is a [good thing](https://csswizardry.com/2013/04/shame-css/).
...
An unusual, but potentially very useful namespace is this one, for your QA team. When running automated UI tests with something like [Selenium](http://www.seleniumhq.org/), or a headless browser, it is quite common to do something like:
...
![](https://csswizardry.com/wp-content/uploads/2015/03/autocomplete-anim.gif)

Animated GIF showing class name autocompletion. [View full size/quality (88KB).](https://csswizardry.com/wp-content/uploads/2015/03/autocomplete-anim-full.gif)
...
Yes, that’s very icky. I’ve never really written any regex before, so I have absolutely no doubt at all that there is a much more terse and effective way to achieve the same thing, but for now this regex seems to work for (almost) all eventualities: [try it out](https://regex101.com/r/rG7uF4/5).
...
This will yield all Theme namespaces (the `\` is simply escaping the `.` so that it matches the `.` string, and not its regex meaning of _anything_) in our source CSS files.
...
For reference, [here’s a particularly useful cheat sheet](http://www.viemu.com/a_vi_vim_graphical_cheat_sheet_tutorial.html) I referred to when I began to [learn Vim](https://csswizardry.com/2014/06/vim-for-people-who-think-things-like-vim-are-weird-and-hard/).
...
1.  It is out of the context of an actual big project, so although it demonstrates what the namespaces are, it’s too small an example to _really_ show how powerful namespacing is.
...
All of that learned _just_ from some rich meaning placed in front of our classes. Amazing.
