...
_Tada!_ We replaced all the `:nth-child()` selectors with three lines of CSS that cover any number of items. Now let’s make it responsive!
...
_But wait!_ That’s no good at all. We are declaring `grid-column-start` twice, so only one of them will get used. We should have only one declaration, and for that, we can combine both conditions using a single [`if()`](https://css-tricks.com/if-css-gets-inline-conditionals/) statement:
