...
source: "https://ics.media/entry/11221/?utm_source=atom&utm_medium=referral&utm_campaign=feed"
...
ふりがな欄については、HTML標準に専用の`autocomplete`値はありません。そのため、`autocomplete`属性には通常の氏名欄と同じ値を指定します。

Chromeでは、`autocomplete`属性・ラベル・`name`属性を手がかりに、ふりがな欄かどうかを判別します。`name="family-name-phonetic"`や`name="given-name-phonetic"`はHTML標準の`autocomplete`値ではなく、Chromeが姓・名のふりがな欄を推定するために参照する`name`属性の文字列です。

かな・カナの判定には、`name`属性ではなくラベルが使われます。ラベルにカタカナが含まれていればカタカナ、含まれていなければひらがなが入力されます。サンプルコードでは、姓と名のふりがな欄を次のように指定しています。

なお、ここで紹介しているふりがな欄の自動入力はChromeのみで利用できます。2026年5月時点では、Microsoft Edgeではこのふりがな欄の自動入力は利用できません。

```
<label>セイ
  <input type="text" name="family-name-phonetic" autocomplete="family-name" placeholder="タナカ" required />
</label>

<label>メイ
  <input type="text" name="given-name-phonetic" autocomplete="given-name" placeholder="タロウ" required />
</label>
```
...
-   氏名は可能であれば姓・名にわけ、`family-name`と`given-name`を指定する。ふりがな欄も同じ単位にわけると判別しやすい
...
各サイトのエントリーフォームを見ていると、サイトにより`name`値の命名規則はさまざまです。たとえば姓を`name1`や`familyname`としているサイトや、郵便番号を`zip`や`postal`、ひどい場合だと`YBBG`（Yu Bin Ban Gou）というパラメーターを見たことがあります。
...
さまざまな値を指定できるので参照ください。サンプルには`inputmode`ごとの仮想キーボードを確認できるページも用意しています。

![キーボードの種類を指定する](https://ics.media/entry/11221/images/images/201201_efo_inputmode.png)

-   [inputmodeのサンプルを別ウインドウで開く](https://ics-creative.github.io/160304_form_autocompletetype/inputmode.html)
...
```
// 再読み込み・タブを閉じる操作の前に、入力内容の破棄確認
addEventListener("beforeunload", (event) => {
  // 既定の移動処理を止めて、ブラウザーの確認ダイアログ表示
...
-   『[HTMLの標準機能で作るフォームバリデーション](https://ics.media/entry/240418/)』