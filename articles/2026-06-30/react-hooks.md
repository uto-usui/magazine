---
title: "React Hooks入門"
source: "https://ics.media/entry/201106/?utm_source=atom&utm_medium=referral&utm_campaign=feed"
...
`useEffect` には第1引数に実行したい関数を指定します。第2引数には依存配列を指定でき、初回の実行に加えて、配列に渡した値が変更されたときにも第1引数の関数を実行します。第2引数を省略すると、レンダリング結果が反映されるたびに実行されます。

#### 実行タイミングを第2引数で制御する

`useEffect`の第2引数に空配列を渡すと、コンポーネントがマウントされたときに実行されます。戻り値としてクリーンアップ関数を返した場合、その関数はアンマウント時に実行されます。

▼ コンポーネントのマウント時に`console.log()`を実行する

```
import React, { useEffect } from "react";
export const AlwaysMountComponent = () => {
  // コンポーネントのマウント時にconsole.log() を呼び出します。
  useEffect(() => {
    console.log("useEffect: コンポーネントがマウントされました");
  }, []);
...
※React 18以降の開発環境のStrict Modeでは、最初のマウント時に`useEffect`のセットアップが追加で1回実行され、クリーンアップ関数がある場合はそれも実行されます。学びはじめで挙動を追いづらい場合は、ReactのStrict Modeを無効にすると学びやすいでしょう。

#### `useEffect`のクリーンアップ

`useEffect`には戻り値に関数を指定できます。ここで指定した処理を「クリーンアップ関数」と呼び、依存配列の値が変わって次のEffectを実行する前と、コンポーネントのアンマウント時に実行されます。次のコードは、`setInterval()`を使った1秒おきに数値が増えるコンポーネントです。コンポーネントの破棄時に`clearInterval()`を実行し、タイマーを解除しています。
...
#### 依存しているstateを`useEffect`内で更新し続けない

以下は一見問題ないように見えますが、依存配列に指定した`state`を`useEffect`内で毎回更新しているため、無限ループになります。
...
今回のように`useEffect`内で以前の`state`の値を使用して一度だけ更新したい場合は、更新用関数を使います。`setCount((count) => count + 1)`のように、更新用関数で以前の値を受け取り、それを元に更新すれば`count`自体を依存配列から削除できます。
...
メモ化は、同じ入力に対して前回の計算結果や値を再利用する最適化です。Reactでは`memo`、`useMemo`、`useCallback`などで、値や関数の参照を再利用できます。

`useCallback`は、依存配列の値が変わるまで同じコールバック関数の参照を返すHooksです。コンポーネントが更新されるたびに新しい関数参照が作られると、`memo`化した子コンポーネントの再レンダリングを防げない場合があります。正しい用途で使用すれば、不要な再レンダリングを抑えられます。
...
`useState`との違いとして、`useRef`で保持している値は、**変更されてもコンポーネントが再レンダリングされない**という特徴があります。「コンポーネントの値を変えたいけど、再レンダリングは行いたくない」というケースで活用するとよいでしょう。次のコードを実行すると、内部の値は変更されていても`<p>`要素の表示は変わっていないことがわかります。
...
`useLayoutEffect`は`useEffect`とほぼ同じ形で使えますが、コールバック関数内の処理とそこで予約した`state`の更新が**ブラウザが画面を再描画する前に処理されることを保証する**ところにおおきな違いがあります。`useEffect`は多くの場合、画面が描画されたあとに実行されるため、DOMの測定結果に応じて表示位置を直すような処理では、補正前の表示が一瞬見えてしまうことがあります。

しかし残念ながらデメリットもあります。画面の再描画をブロックするということは、画面描画完了までの時間が長くなってしまうということです。結果的にどうしてもユーザーへのページ表示が遅くなってしまい、ユーザーの体験を損なってしまいます。描画前にDOMの測定や反映が必要なとき以外は `useEffect`の方を使ったほうが無難でしょう。

公式ドキュメントでも、基本的に`useEffect`の方を使うことが[推奨されています](https://ja.react.dev/reference/react/useLayoutEffect)。

また、プロジェクトでサーバーサイドレンダリングを使用している場合、Effectはサーバー上では実行されません。`useLayoutEffect`はサーバー上ではレイアウト情報を取得できないため、クライアントでのハイドレーション後に動作する前提で設計する必要があります。

`useLayoutEffect`は、ツールチップなどのDOMサイズや位置を測定し、その結果をユーザーに見える前に反映したいケースなどに役立ちます。

### useMemoとuseCallbackの違い

`useMemo`は計算用の関数を第一引数に、`useCallback`はキャッシュしたい関数そのものを第一引数に受け取ります。どちらも第二引数には依存配列を渡します。

`useMemo`は一度計算した結果を変数に保持してくれる「メモ化」を行います。最初のレンダリングで一度作業を行い、第二引数に渡した依存する値が変化しない限りはキャッシュされたものを返します。それに対して`useCallback`は、第一引数に渡した**コールバック関数**の参照を保持します。`useCallback`自体が再レンダリングを止めるわけではありませんが、`memo`化した子コンポーネントなどに渡す関数の参照を安定させられます。

`useMemo`はコストの高い計算結果の再利用に、`useCallback`はメモ化した子コンポーネントや他のHooksの依存値として関数の参照を安定させたいときに使用するとよいでしょう。単に同じコンポーネント内のイベントハンドラーとして使うだけなら、効果はほとんどないので気をつけましょう。

```
// 効果が薄い: メモ化したコンポーネントに渡さず、そのまま利用
const Component = () => {
  const handleClick = useCallback(() => { ... }, []);
  return (
...
[React Compiler](https://ja.react.dev/learn/react-compiler)は、コンポーネントや値を自動的に最適化し、`useMemo`、`useCallback`、`React.memo`による手動メモ化が必要になる場面を減らす仕組みです。導入すると、不要な再計算や再レンダリングを抑えつつ、メモ化のためだけに書くコードや依存配列の管理を減らせます。

一方で、React Compilerはビルド時の仕組みなので、導入していないプロジェクトでは従来どおり`useMemo`や`useCallback`が必要になる場面があります。また、React Compilerが安全に最適化するためにも、Hooksのルールやコンポーネントの純粋性を守ることが重要です。
...
export const StateComponent = ({ enabled }) => {
  ...
  if (enabled) {
    const [value2, setValue2] = useState("fuga");
  }
  // ...省略...
};
```

状況によって処理を分岐させたいときは、Hook自体はトップレベルで呼び出し、そのあとで処理を分岐させましょう。
...
公式で[eslint-plugin-react-hooks](https://ja.react.dev/reference/eslint-plugin-react-hooks)という、Hooksをルールどおり正しく使えているかどうかをチェックするプラグインを提供してくれていますので、これを活用するとよいでしょう。`eslint-plugin-react-hooks`はReact Compilerが検出する問題も扱うため、React Compilerを導入していないプロジェクトでも、将来的な最適化に備えたコード品質の確認に役立ちます。