...
source: "https://ics.media/entry/14973/?utm_source=atom&utm_medium=referral&utm_campaign=feed"
...
※記事の末尾に各種ライブラリの容量やライセンス等を記載しています
...
GSAPは73KBと容量がやや大きいものの、ツリーシェイキングに対応。パッケージマネージャーnpmやTypeScriptでの利用も対応しています。

GSAPはFlash時代から長期間にわたり継続的に開発されています。[GitHubの履歴](https://github.com/greensock/GSAP/graphs/contributors)をみても高い頻度で更新されています。

GSAPは商用利用も含め100%無料です。以前は、特定の用途では有料ライセンスが必要でしたが、2025年4月のv3.13よりボーナスプラグインも含めて無料利用できるようになりました。詳しくは公式サイトの次のページをご覧ください。
...
-   [アニメーション最強のライブラリGSAP](https://ics.media/entry/200805/)
...
ReactやVueに宣言的UIとして組み込むことができるほか、生のJavaScriptとしても利用できます。npmでもダウンロード数が多いことも特徴です（参照：[npm trends](https://npmtrends.com/animejs-vs-gsap-vs-motion)）。
...
今回の同時実行性能の検証では、MotionはGSAPやAnime.jsほど高い結果ではありませんでした。また、演出系の機能やプラグインの多さではGSAPやAnime.jsに軍配が上がります。

ICS MEDIAでは次の記事で詳しく説明しています。

-   [ReactとMotionでつくる! “少し”こだわったマイクロインタラクション集](https://ics.media/entry/251204/)
...
[Anime.js](https://animejs.com/)は2016年に登場したJSライブラリです。登場時に公開された[デモ（当時は公式サイト掲載されていた）](https://codepen.io/juliangarnier/pen/xOgyjB)のオシャレな表現が[注目を集めました](https://b.hatena.ne.jp/entry/anime-js.com/)。[公式サイト](https://animejs.com/)（2025年にリニューアル）にはセンスのよい演出が施されています。海外製のライブラリなのに、「アニメ」という名前が採用されているのが日本のエンジニアとして嬉しいですね。[ドキュメントのデモ](https://animejs.com/documentation/)がわかりやすいのもよい特徴でしょう。
...
Anime.jsは2024年頃にバージョン4が公開され、2026年現在も積極的な開発が続いています。以前のバージョンでは、GSAPやTweenJSに比べて同時実行性能が劣っていましたが、バージョン4になり同水準の性能に達しています。
...
[Web Animations API](https://www.w3.org/TR/web-animations-1/)はDOM要素をアニメーションできるAPIです。全ブラウザ（Chrome、Edge、Firefox、Safari）が対応しています。
...
※容量は代表的な配布ファイルのファイルサイズを記載（GZIPではありません）
...
ライブラリに頼らず、CSSの`linear()`関数でスプリングアニメーションを実装したい場合は、記事「[CSSのlinear()でUIが軽快になる! スプリングアニメーション活用術](https://ics.media/entry/260402/)」も参考になります。DOM中心の小さなUIなら、CSSだけで十分なケースもあります。

続編記事「[JSライブラリの性能をDOMとWebGPUで比較検証](https://ics.media/entry/14993/)」で各ライブラリのパフォーマンス（同時実行性能）を紹介しています。こちらも合わせてご覧ください。