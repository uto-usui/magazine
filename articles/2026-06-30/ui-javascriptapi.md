...
source: "https://ics.media/entry/230530/?utm_source=atom&utm_medium=referral&utm_campaign=feed"
...
3.  ボタンで行いたい動作を`popovertargetaction`属性に指定します。今回はポップオーバーを開閉したいので`toggle`を使います。

※`popovertargetaction`を省略した場合はデフォルト値が`toggle`になります。省略しても動きは同じですが、今回は明示的に指定します。

```
<!-- ▼ポップオーバーを制御するボタン -->
<button popovertarget="popover1" popovertargetaction="toggle">Popover 1</button>
<!-- ▼ポップオーバー本体 -->
...
<!-- ▼ポップオーバーを制御するボタン -->
<button popovertarget="popover2" popovertargetaction="toggle">Popover 2</button>
<!-- ▼ポップオーバー本体 -->
...
1つめの例ではHTMLに`popover`や`popovertarget`、`popovertargetaction`といった属性を指定することで、ポップオーバーの表示/非表示を実装しました。今回はJavaScriptを使用して手動で表示/非表示を切り替えてみます。
...
### コラム: `popovertarget`/`popovertargetaction`と`command`/`commandfor`はどう違う？

ポップオーバーを操作するための属性には以下2つのペアがあります。

-   `popovertarget`/`popovertargetaction`
-   `command`/`commandfor`

どちらも宣言的にポップオーバーの振る舞いを記載できます。記事の冒頭では`popovertarget`を使った例を紹介しましたが、以下のように記載しても同じ動きになります。

```
<button commandfor="popover1" command="toggle-popover">Popover</button>
<div id="popover1" popover class="popover-content">
  <p>commandとcommandforを使った例</p>
</div>
```

`popovertarget`/`popovertargetaction`はポップオーバーにしか使えないのに対し、`command`/`commandfor`はダイアログにも使えるより汎用的な属性という位置付けです。`command`/`commandfor`について詳しく知りたい方は記事『[モーダルやツールチップで役立つ! HTMLのcommandとinterestfor属性を使って、JSを減らすスマートなUI開発](https://ics.media/entry/260209/)』にも記載があるので参考にしてください。
...
ポップオーバーやトーストの出現にスプリングアニメーションを加えたい場合は、記事「[CSSのlinear()でUIが軽快になる! スプリングアニメーション活用術](https://ics.media/entry/260402/)」も参考になります。
