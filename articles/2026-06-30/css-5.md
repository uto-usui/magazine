...
source: "https://ics.media/entry/15649/?utm_source=atom&utm_medium=referral&utm_campaign=feed"
...
### 高さが異なるカードを詰めて配置するGrid Lanes

ここまでのサンプルは、行と列の両方をそろえる通常のGrid Layoutを前提にしていました。カードごとに高さが異なるレイアウトでは、行の高さに引っ張られて空きが生まれやすくなります。写真ギャラリーやカード一覧のように、高さがばらつく要素をすき間なく並べたい場面では、Grid Lanesグリッドレーンが役立ちます。

-   [サンプルを別ウインドウで開く](https://ics-creative.github.io/260325_grid_lanes/index.html)
-   [コードを確認する](https://github.com/ics-creative/260325_grid_lanes/blob/main/index.html)

Grid Lanesは、列または行のどちらか一方だけを定義し、もう一方の方向にはアイテムを詰めながら配置するレイアウトです。次のサンプルでは`grid-template-columns`で**列だけ**を定義しているため、ブラウザーは各カードの高さを見ながら、前方に置ける列へ順にカードを流し込みます。通常の`display: grid;`のように**行をそろえない**ため、高さの異なるカードを並べても余白が目立ちにくくなります。

-   [サンプルを別ウインドウで開く](https://ics-creative.github.io/260325_grid_lanes/simple-sample.html)
-   [コードを確認する](https://github.com/ics-creative/260325_grid_lanes/blob/main/simple-sample.html)

```
.grid-lanes-demo {
  display: grid-lanes; /* グリッドレーンを指定 */
  grid-template-columns: repeat(auto-fit, minmax(112px, 1fr));
  flow-tolerance: 16px; /* アイテムを詰めるときの許容範囲 */
  gap: 8px; /* アイテム間の余白 */
}
```
...
Grid LayoutやFlexboxの`gap`プロパティに対して`rule`プロパティで装飾できます。2026年6月のChrome 149から利用可能になりました。
...
.container {
  ...
  gap: 24px;
  rule: 2px solid black; /* gapを装飾 */
}
```

-   [サンプルを別ウインドウで開く](https://ics-creative.github.io/170901_css_grid_layout/gap-rule/rule.html)
-   [コードを確認する](https://github.com/ics-creative/170901_css_grid_layout/blob/master/gap-rule/rule.html)

列方向と行方向のgapを個別に装飾したい場合は`column-rule`と`row-rule`を使います。行と列の装飾が交差する部分は`rule-break`プロパティで制御でき、`intersection`で交差部分で線が途切れさせることができます。

```
.container {
  column-rule: 2px solid black; /* 列のgapを装飾 */
  row-rule: 2px dashed black; /* 行のgapを装飾 */
  rule-break: intersection; /* 交差部分で線を途切れさせる */
}
```

-   [サンプルを別ウインドウで開く](https://ics-creative.github.io/170901_css_grid_layout/gap-rule/rule_column.html)
-   [コードを確認する](https://github.com/ics-creative/170901_css_grid_layout/blob/master/gap-rule/rule_column.html)

#### 作例：スケジュール表

次のサンプルでは、スケジュール表の罫線として`gap`を装飾しました。従来のCSSだと表現が難しかったことがCSS Gridの`rule`で容易に実現可能になったのは嬉しいですね！

-   [サンプルを別ウインドウで開く](https://ics-creative.github.io/170901_css_grid_layout/gap-rule-schedule/)
-   [コードを確認する](https://github.com/ics-creative/170901_css_grid_layout/tree/master/gap-rule-schedule)

CSS Gap Decorationsの詳しい使い方や作例は、記事『[gapに線をひきたい! gridとflexに罫線を入れられる新しいCSS](https://ics.media/entry/260618/)』で解説しています。
...
サブグリッドはChrome・Edge 117（2023年9月）、Safari 16.0（2022年9月）、Firefox 71（2019年12月）以上で利用できます。

-   参考：『[CSS Subgrid | Can I use...](https://caniuse.com/css-subgrid)』

`rule`プロパティは、Chrome・Edge 149（2026年6月）以上で利用できます。

参照：[CSS property: rule-break | Can I use...](https://caniuse.com/mdn-css_properties_rule-break)

グリッドレーンは、Safari 26.4（2026年3月）以上で利用できます。
