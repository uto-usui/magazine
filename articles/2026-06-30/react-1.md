---
title: "React今昔物語"
source: "https://ics.media/entry/200310/?utm_source=atom&utm_medium=referral&utm_campaign=feed"
...
ReactはFacebook（現Meta）の社内プロジェクトから生まれたライブラリです。現在はReact Foundationへ移管されています。
...
ライセンスがBSD + PatentsからMITライセンスに変更。  
Reactバージョン16.0.0の発表。  
...
`componentWill~`系のライフサイクルメソッドが非推奨になる
...
[React Compiler 1.0](https://react.dev/blog/2025/10/07/react-compiler-1)のリリース  
ビルド時の自動メモ化による最適化

2026年2月

[React Foundation](https://react.dev/blog/2026/02/24/the-react-foundation)が正式に発足  
React、React Native、JSXなどがMetaからReact Foundationへ移管

2026年6月

GitHub上のReact本体のリポジトリが`facebook/react`から[`react/react`](https://github.com/react/react)へ移管

機能改善だけでなく、非推奨になった機能や運営体制の変化もありますね。
...
React + TypeScriptでの新規開発の際は、記事『[最新版TypeScript+webpack 5の環境構築まとめ(React, Three.jsのサンプル付き)](https://ics.media/entry/16329/)』を参照ください。
...
Reactの基本として、データはpropsからトップダウン（親から子）で渡されます。 そのため、必要なコンポーネントまでデータを共有するいわゆる「propsのバケツリレー」を行う必要があります。 たとえばUIテーマの切り替え情報や言語設定など、アプリケーションの中で多くのコンポーネントから必要とされているプロパティを渡すのはとても手間になります。
...
ただし、前述のError Boundaryに利用するライフサイクルメソッドである、`componentDidCatch`および`getDerivedStateFromError`に関しては現状のバージョンのHooks APIで実装されていません。
...
Hooksはとても便利ですが、第二引数（`deps`）に正しい参照対象を渡さないと、処理が無限ループしてしまったりする危険性もあります。 解決策としてESLintのプラグイン「[eslint-plugin-react-hooks](https://react.dev/reference/eslint-plugin-react-hooks)」を活用するのもよいでしょう。Hooksの参照対象の誤りを警告してくれます。
...
また、2021年10月にはFacebookがMetaへ社名を変更しました。Reactは引き続きMetaが主導するOSSとして開発されていました。
...
### 2025年〜　React Compilerが安定版に

2025年10月に[React Compiler 1.0](https://react.dev/blog/2025/10/07/react-compiler-1)がリリースされ、安定版として利用できるようになりました。

React CompilerはReactの実行時APIではなく、ビルド時にReactアプリを解析して最適化するツールです。コンポーネントやHooksを自動でメモ化するため、手動で`useMemo`、`useCallback`、`React.memo`を使う場面を減らせます。

また、React Compiler由来のLintルールは`eslint-plugin-react-hooks`に統合されています。

### 2026年〜　React Foundationへの移管

Reactは長らくFacebook（現在のMeta）が主導してきましたが、2026年2月にLinux Foundationのもとで[React Foundation](https://react.dev/blog/2026/02/24/the-react-foundation)が正式に発足しました。React、React Native、JSXなどの関連プロジェクトはMetaの所有ではなくなり、独立したReact Foundationの所有となっています。

この移管にあわせて、2026年6月にGitHub上のReact本体のリポジトリも`facebook/react`から[`react/react`](https://github.com/react/react)へ移管されました。
...
ICS MEDIAでは他にも、『[React Hooks入門](https://ics.media/entry/201106/)』や『[Reactで作る、WAI-ARIA対応のアクセシブルなタブ型UI](https://ics.media/entry/17109/)』で前述のHooks APIの使い方をソースコード付きで紹介しています。こちらも合わせてご覧ください。