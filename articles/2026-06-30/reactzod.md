...
source: "https://ics.media/entry/240611/?utm_source=atom&utm_medium=referral&utm_campaign=feed"
...
前回の記事[『HTMLの標準機能で作るフォームバリデーション』](https://ics.media/entry/240418/)ではHTMLの機能を駆使したフォームバリデーションの実装について解説しました。HTMLのみでも高機能なフォームを作成できるのは解説したとおりですが、HTMLに加えてJavaScriptを組み合わせることでより高機能なフォームを作成できます。それに加えて、**開発者体験の向上**も期待できます。
...
※本記事のサンプルコードではTypeScript 6、React 19、Zod 4を使用します。
...
`z.string()`や`z.number()`、`z.int()`などはプリミティブな型をバリデーションするスキーマです。その後にそれぞれの型に対するスキーマをつなげられます。Zod 4では整数のバリデーションに`z.number().int()`ではなく、トップレベルの`z.int()`を使うのが推奨されています。

`.min(1)`とつなげると1文字以上の入力が必須、つまり入力必須のバリデーションをかけられます。さらに、バリデーションメッセージも`error`フィールドで独自に定義できます。

```
/** 名前 */
z.string().min(1, { error: "名前を入力してください" });
```

数値型に対するスキーマでは、整数型や最小値、最大値のバリデーションが用意されています。数値かどうかを確認してから整数に絞り込む場合は`z.pipe()`で`z.number()`と`z.int()`をつなぎ、最小値のチェックには`.refine()`を使います。
...
整数のみを扱うシンプルなケースでは、`z.int().gte()`のように`z.int()`から直接チェーンできます。

```
z.int({ error: "年齢を整数で入力してください" }).gte(12, { error: "年齢を12歳以上で入力してください" });
```

「2つのスキーマのうちどちらかを満たせばよい」場合もあるでしょう。その場合は`z.union()`スキーマが便利です。次の例は、「空文字もしくはメールアドレス形式の文字列型」のスキーマ定義です。未入力（`undefined`）も許容する場合は`.optional()`をつけます。
...
文字列型には`z.email()`や`z.url()`といったトップレベルのフォーマットバリデータのほか、`.startsWith()`などのメソッドチェーンや正規表現などで独自にスキーマ定義もできます。
...
日付形式で入力してほしいので、`z.iso.date()`スキーマを使用します。そのあとに続く`.refine()`スキーマはオブジェクトスキーマ全体のバリデーションを行う処理です。第一引数にはバリデーション関数を記述しますが、引数からそれぞれのフィールドにアクセスできます。今回は`startDate`が`endDate`よりも小さければよいので、比較の処理を記述します。
...
ネストしたスキーマにアクセスする場合は`date.startDate`のように子階層のプロパティにアクセスします。ここでも型補完が効きます。
...
React 19では**フォームアクション**周りの機能が充実しました。

フォームは入力して終わりではなく、値をサーバーに送信し正しいレスポンスをもらって処理を終えます。また、非同期な処理になるので送信中を示すようなUIの表示、送信失敗時のケアなどの仕組みも必要です。React 19ではこれらの実装を容易にする機能が提供されています。
...
これまで開発者が自前でケアしてきたような実装がライブラリの機能でできるようになりました。

具体的な機能に興味のある方は、React公式ブログの記事を参照ください。
