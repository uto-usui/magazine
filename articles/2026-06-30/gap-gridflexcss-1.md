...
source: "https://ics.media/entry/210628/?utm_source=atom&utm_medium=referral&utm_campaign=feed"
publishedDate: "2021-06-27"
category: "frontend"
feedName: "ICS MEDIA"
author: "ics"
---
...
-   [ソースコードを確認する](https://github.com/ics-creative/210628_gap_layout)
...
`gap`プロパティーは1つもしくは2つの値をとり、それぞれ行間・列間のアキを設定します。1つ指定の場合は行間・列間を同一の値で指定します。2つの場合は、1つ目が行間、2つ目が列間の値になります。`gap`プロパティーは一括指定プロパティーなので、行間・列間を個別に指定する場合はそれぞれ`row-gap`、`column-gap`を使います。

![gapプロパティーが制御するアキ](https://ics.media/entry/210628/images/210628_row-column-gap.png)

CSS Gridでの`gap`の使い方や、`rule`プロパティーによるgapの装飾については[『CSS Grid Layout入門』](https://ics.media/entry/15649/)の記事で詳しく解説しています。
...
![全体のレイアウト](https://ics.media/entry/210628/images/210628_laytout-vh.png)
...
![](https://ics.media/entry/210628/images/210628_naviagtion.png)
...
2026年6月のChrome・Edge 149からは、gap部分に線を引けるCSS Gap Decorationsも利用できます。詳しくは記事『[gapに線をひきたい! gridとflexに罫線を入れられる新しいCSS](https://ics.media/entry/260618/)』をご覧ください。
...
レスポンシブで列数が変わるレイアウトを考えてみましょう。`margin`プロパティを使った場合、3列のレイアウトの場合、2列の場合、とメディアクエリを使用してクラスを書き換える必要があります。
...
`gap`プロパティーを使えば列数に関係なく1行で記載できます。
...
![親要素が子要素の位置を決定する](https://ics.media/entry/210628/images/210628_parent-child-gap.png)
...
Figmaには「Auto layout」という機能があります。この挙動はFlexboxと`gap`プロパティーを組み合わせたもの近いです。デザインデータと同じような感覚でレイアウトができ、デザイナーの意図も汲み取りやすいでしょう。
