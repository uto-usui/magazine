...
source: "https://ics.media/entry/7298/?utm_source=atom&utm_medium=referral&utm_campaign=feed"
...
※本記事はElectron 42.3.0、Node.js 24を対象に解説しています
...
採用事例には「[Visual Studio Code](https://code.visualstudio.com/) (Microsoft)」「[Slack](https://slack.com/)」「[Discord](https://discord.com/)」「[Figma Desktop](https://www.figma.com/downloads/)」などがあります。大企業も採用していることから、導入への不安が少なくなりますね。
...
アプリケーションを作成するために、`package.json`の編集と4つのファイルを用意します。以下にて、それぞれどのようなファイルなのかを説明します。

![](https://ics.media/entry/7298/images/260603_electron_samples.png)
...
メインウィンドウとして開くHTMLファイルを用意します。`main.js`でメインウィンドウとして`index.html`ファイルを指定しています。
...
Electronはメインプロセスとレンダラープロセスの2つがあり、画面側のレンダラープロセスからは自由にElectronのAPIを実行することができません。プリロードスクリプトと`contextBridge`はその課題を解決する手段のひとつです。

プリロードスクリプトは、レンダラープロセスが読み込まれる前に実行されます。`contextBridge` APIを使うと、選んだ機能だけをレンダラープロセスから安全に利用できます。

![Electronのメインプロセス・プリロード・レンダラーの関係](https://ics.media/entry/7298/images/260603_process.png)
...
レンダラープロセス側のJavaScriptは`renderer.js`に記述します。プリロードスクリプトが公開した`versions`を使って、画面にバージョン情報を表示します。

▼ `renderer.js`ファイル

```
const information = document.getElementById("info");
information.innerText = `We are using Node.js (v${versions.node()}), Chromium (v${versions.chrome()}), and Electron (v${versions.electron()}).`;
```

たくさんファイルを作成しましたが、ここでフォルダーを再確認しておきましょう。

![Electronにおけるpackage.jsonファイルの役割](https://ics.media/entry/7298/images/260603_electron_samples.png)
...
このscriptsは`npm run start`のように実行できます。コマンドを短く覚えやすくできるため、開発効率が向上します。npm scriptsの使い方は記事「[Node.jsユーザーなら押さえておきたいnpm-scriptsのタスク実行方法まとめ](https://ics.media/entry/12226/)」に詳しくまとめています。
